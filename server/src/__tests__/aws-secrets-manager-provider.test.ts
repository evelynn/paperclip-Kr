...
describe("awsSecretsManagerProvider", () => {
  ...
  it("creates AWS secrets from selected provider vault config without deployment env fallback", async () => {
    ...
    // Isolate from ambient AWS credentials (present in some CI/hosted envs):
    // the health check warns when static creds are visible, which would flip
    // the asserted status from "ok" to "warn".
