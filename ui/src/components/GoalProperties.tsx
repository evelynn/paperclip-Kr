  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("projectsComp.goalProperties.status")}>
          {onUpdate ? (
            <PickerButton
              current={goal.status}
        ...
        <PropertyRow label={t("projectsComp.goalProperties.level")}>
          {onUpdate ? (
            <PickerButton
              current={goal.level}
              options={GOAL_LEVELS}
        ...
        <PropertyRow label={t("projectsComp.goalProperties.owner")}>
          {ownerAgent ? (
            <Link
            ...
            <span className="text-sm text-muted-foreground">{t("projectsComp.goalProperties.none")}</span>
          )}
        </PropertyRow>

        {goal.parentId && (
          <PropertyRow label={t("projectsComp.goalProperties.parentGoal")}>
            <Link
      ...
      <div className="space-y-1">
        <PropertyRow label={t("projectsComp.goalProperties.created")}>
          <span className="text-sm">{formatDate(goal.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("projectsComp.goalProperties.updated")}>
