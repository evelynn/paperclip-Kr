...
export function MobileBottomNav({ visible }: MobileBottomNavProps) {
  ...
  const items = useMemo<MobileNavItem[]>(
    () => [
      { type: "link", to: "/dashboard", label: t("nav.mobile.home"), icon: House },
      { type: "link", to: "/issues", label: t("nav.items.tasks"), icon: CircleDot },
      { type: "action", label: t("nav.mobile.create"), icon: SquarePen, onClick: () => openNewIssue() },
      { type: "link", to: "/agents/all", label: t("nav.agents.section"), icon: Users },
      {
        type: "link",
        to: "/inbox",
