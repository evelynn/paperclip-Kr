...
export function NewAgentDialog() {
  ...
  function handleAskCeo() {
    ...
    openNewIssue({
      ...
      title: t("agentsPage.dialog.askCeoTitle"),
      description: t("agentsPage.dialog.askCeoDescription"),
    });
  }
  ...
  async function copyText(text: string, unavailableBody: string) {
    ...
    pushToast({
      title: t("agentsPage.dialog.clipboardUnavailableTitle"),
  ...
  const createAgentInviteMutation = useMutation({
    ...
    onSuccess: async (invite) => {
      ...
      setLatestAgentPromptCopied(false);
      setMode("prompt");
      const copied = await copyText(prompt, t("agentsPage.dialog.clipboardUnavailableBelow"));

      await queryClient.invalidateQueries({ queryKey: inviteHistoryQueryKey });
      pushToast({
        title: t("agentsPage.dialog.inviteCreatedTitle"),
        body: copied ? t("agentsPage.dialog.promptReadyCopied") : t("agentsPage.dialog.promptReady"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("agentsPage.dialog.inviteFailedTitle"),
        body: error instanceof Error ? error.message : t("agentsPage.dialog.unknownError"),
        tone: "error",
      });
    },
  });
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <span className="text-sm text-muted-foreground">{t("agentsPage.dialog.addAgent")}</span>
          <Button
            variant="ghost"
            size="icon-xs"
          ...
          {mode === "choices" ? (
            ...
            <>
              <div className="space-y-2">
                ...
                >
                  ...
                  {t("agentsPage.common.back")}
                </button>
                <p className="text-sm text-muted-foreground">
                  {t("agentsPage.dialog.chooseRuntime")}
                </p>
              </div>
                  ...
                  >
                    {opt.recommended && (
                      <span className="absolute -top-1.5 right-1.5 bg-green-500 text-white text-[9px] font-semibold px-1.5 py-0.5 rounded-full leading-none">
                        {t("agentsPage.dialog.recommended")}
                      </span>
                    )}
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <h2 className="text-sm font-semibold">{t("agentsPage.dialog.inviteExternal")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("agentsPage.dialog.inviteIntro")}
                  </p>
                </div>
              </div>

              <label className="block space-y-2">
                <span className="text-sm font-medium">{t("agentsPage.dialog.optionalMessage")}</span>
                <Textarea
                  value={agentMessage}
                  onChange={(event) => setAgentMessage(event.target.value)}
                  className="min-h-24 resize-y"
                  placeholder={t("agentsPage.dialog.optionalMessagePlaceholder")}
                  maxLength={4000}
                />
              </label>

              <div className="rounded-lg border border-border px-4 py-3 text-sm text-muted-foreground">
                {t("agentsPage.dialog.inviteApprovalNote")}
              </div>
              ...
              <div>
                ...
                >
                  {createAgentInviteMutation.isPending ? t("agentsPage.dialog.generating") : t("agentsPage.dialog.generatePrompt")}
                </Button>
              </div>
            </div>
          ) : (
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-3">
                    <h2 className="text-sm font-semibold">{t("agentsPage.dialog.promptTitle")}</h2>
                    {latestAgentPromptCopied ? (
                      <div className="inline-flex items-center gap-1 text-xs font-medium text-foreground">
                        <Check className="h-3.5 w-3.5" />
                        {t("agentsPage.dialog.copied")}
                      </div>
                    ) : null}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {t("agentsPage.dialog.promptHint")}
                  </p>
                </div>
              </div>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const copied = await copyText(latestAgentPrompt, t("agentsPage.dialog.clipboardUnavailableAbove"));
                  setLatestAgentPromptCopied(copied);
                }}
              >
                {latestAgentPromptCopied ? t("agentsPage.dialog.copiedPrompt") : t("agentsPage.dialog.copyPrompt")}
              </Button>
            </div>
          )}
        </div>
