    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={t("issues.columns.picker")}
        >
          <Columns3 className={iconOnly ? "h-3.5 w-3.5" : "mr-1 h-3.5 w-3.5"} />
          {!iconOnly && t("issues.columns.picker")}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
          ...
          >
            <span className="flex flex-col gap-0.5">
              <span className="text-sm font-medium text-foreground">
                {t(`issues.columns.title.${column}`)}
              </span>
              <span className="text-xs leading-relaxed text-muted-foreground">
                {t(`issues.columns.description.${column}`)}
              </span>
            </span>
          </DropdownMenuCheckboxItem>
        ))}
        ...
        >
          {t("issues.columns.resetDefaults")}
          <span className="ml-auto text-xs text-muted-foreground">{t("issues.columns.resetHint")}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
    ...
    <>
      ...
      {!isLive && subtreeLiveCount > 0 && (
        <span
          ...
          title={t(subtreeLiveCount === 1 ? "issues.board.runningBelowOne" : "issues.board.runningBelowOther", { count: subtreeLiveCount })}
        >
          <span
            className={cn(
          ...
          <span className="hidden text-[11px] font-medium text-muted-foreground sm:inline">
            {t("issues.board.liveBelow", { count: subtreeLiveCount })}
          </span>
        </span>
      )}
    </>
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("issues.columns.userFallback");
              ...
              {workspaceId && onFilterWorkspace ? (
                <Tooltip>
                  ...
                  <TooltipContent side="top" sideOffset={6}>
                    {t("issues.columns.filterByWorkspace")}
                  </TooltipContent>
                </Tooltip>
              ) : (
                workspaceName
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("issues.columns.subTask")}</span>
              )}
            </span>
          );
        }
