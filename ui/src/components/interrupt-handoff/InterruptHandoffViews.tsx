...
function userLabel(userId: string, resolvers: HandoffChipResolvers): string {
  ...
  const base = label ?? t("threadCards.actorBoard");
  return resolvers.currentUserId && resolvers.currentUserId === userId ? t("threadCards.handoff.you", { label: base }) : base;
}
...
export function AssigneeChip({
  ...
  if (assignee.agentId) {
    return (
      <span className={cn(CHIP_CLASS, className)} data-testid="handoff-assignee-chip" data-kind="agent">
        <span className="sr-only">{t("threadCards.handoff.agentPrefix")}</span>
  ...
  if (assignee.userId) {
    return (
      <span className={cn(CHIP_CLASS, className)} data-testid="handoff-assignee-chip" data-kind="user">
        <span className="sr-only">{t("threadCards.handoff.userPrefix")}</span>
  ...
  return (
    ...
    >
      <span className="sr-only">{t("threadCards.handoff.noAssignee")}</span>
      {t("threadCards.handoff.unassigned")}
    </span>
  );
}
...
export function HandoffWakeRow({
  ...
  return (
    <div
      ...
      data-kind={info.kind}
    >
      <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t("threadCards.handoff.wake")}</span>
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        {t("threadCards.handoff.didYouMean")} <span className="font-medium">@{candidate.matchedText}</span>{t("threadCards.handoff.plainTextWontNotify")}
      </span>
      <button
        type="button"
        onClick={onInsert}
        ...
        aria-label={t("threadCards.handoff.insertMentionAria", { name: agentDisplayName })}
      >
        {t("threadCards.handoff.insertMention")}
      </button>
      <button
        type="button"
        onClick={onDismiss}
        className="shrink-0 rounded p-0.5 hover:bg-amber-100/60 dark:hover:bg-amber-500/20"
        aria-label={t("threadCards.handoff.dismissSuggestion")}
      >
      ...
      <div className="flex items-start gap-1.5">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <p className="flex flex-wrap items-center gap-1 text-amber-700/90 dark:text-amber-300/90">
            <span>{t("threadCards.handoff.handOffTo")}</span>
  ...
  return (
    ...
    >
      ...
      {visibleBuckets.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("threadCards.handoff.noTasksAffected")}</p>
      )}
    </div>
  );
}
