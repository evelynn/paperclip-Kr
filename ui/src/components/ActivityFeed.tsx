...
const FILTER_OPTIONS: Array<{ value: FilterValue; labelKey: string }> = [
  { value: "all", labelKey: "feedComp.activityFeed.filter.all" },
  { value: "in-progress", labelKey: "feedComp.activityFeed.filter.inProgress" },
  { value: "for-review", labelKey: "feedComp.activityFeed.filter.inReview" },
  { value: "completed", labelKey: "feedComp.activityFeed.filter.done" },
];
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("feedComp.actor.system")
      : group.latestEvent.actorType === "user" ? t("feedComp.actor.board")
      : t("feedComp.actor.unknown"));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-[#959596]">{t("feedComp.activityFeed.madeUpdates", { count: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
        ...
        separator = (
          <div className="flex items-center gap-2 px-4 py-1.5" key={`sep-${index}`}>
            ...
            <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
              {t("feedComp.activityFeed.earlier")}
            </span>
            <div className="h-px flex-1 bg-border" />
          </div>
        );
  ...
  const renderGrouped = () => {
    ...
    return Array.from(groups.entries()).map(([groupKey, items]) => {
      ...
      const label = isOther
        ? t("feedComp.activityFeed.otherActivity")
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("feedComp.activityFeed.empty.noAgents"),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("feedComp.activityFeed.empty.allPaused"),
        showPulse: false,
      };
    }
    return {
      text: t("feedComp.activityFeed.empty.running"),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("feedComp.activityFeed.title")}</h3>
          <p className="text-xs text-muted-foreground">
            {t("feedComp.activityFeed.subtitle")}
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("feedComp.activityFeed.ariaGroupByTask")}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat" ? t("feedComp.activityFeed.groupByTask") : t("feedComp.activityFeed.showFlat")}
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("feedComp.activityFeed.ariaFilterBy")}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("feedComp.activityFeed.filterBy")}</TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                {t("feedComp.activityFeed.showAllActivity")}
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-[16rem]">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("feedComp.activityFeed.empty.fallback")}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
