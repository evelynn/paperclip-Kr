  ...
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1 rounded-sm border border-emerald-500/50 bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-900 dark:text-emerald-100">
        ...
        {t("issueRelated.decompositions.statusCompleted")}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-sm border border-amber-500/50 bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium text-amber-900 dark:text-amber-100">
      ...
      {t("issueRelated.decompositions.statusInFlight")}
    </span>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-medium text-muted-foreground">{t("issueRelated.decompositions.heading")}</h3>
        <span className="text-[11px] text-muted-foreground/80">
          {t(
            items.length === 1
              ? "issueRelated.decompositions.revisionsCountOne"
              : "issueRelated.decompositions.revisionsCountOther",
            { count: items.length },
          )}
        </span>
      </div>
      ...
      <ul className="space-y-3">
        {items.map((record) => {
          ...
          const ownerName = record.ownerAgentId
            ? agentMap?.get(record.ownerAgentId)?.name ?? t("issueRelated.decompositions.ownerAgentFallback")
            : null;
          const revisionLabel =
            record.acceptedPlanRevisionNumber != null
              ? t("issueRelated.decompositions.planRevisionNumber", {
                  number: record.acceptedPlanRevisionNumber,
                })
              : t("issueRelated.decompositions.planRevisionId", {
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  ...
                  {t(
                    requested === 1
                      ? "issueRelated.decompositions.childCountOne"
                      : "issueRelated.decompositions.childCountOther",
                    { created, requested },
                  )}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  <span
                    ...
                    title={t("issueRelated.decompositions.idempotentClaimTitle")}
                  >
                    <Repeat className="h-3 w-3" />
                    {t("issueRelated.decompositions.idempotentClaim")}
                  </span>
                ) : null}
              </div>

              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-muted-foreground">
                {ownerName ? <span>{t("issueRelated.decompositions.owner", { name: ownerName })}</span> : null}
                {startedAt ? (
                  <span title={formatDateTime(startedAt)}>
                    {t("issueRelated.decompositions.started", { time: relativeTime(startedAt) })}
                  </span>
                ) : null}
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>
                    {t("issueRelated.decompositions.completed", { time: relativeTime(completedAt) })}
                  </span>
                ) : updatedAt ? (
                  <span title={formatDateTime(updatedAt)}>
                    {t("issueRelated.decompositions.updated", { time: relativeTime(updatedAt) })}
                  </span>
                ) : null}
                {issueIdentifier ? (
                  <Link
                  ...
                  >
                    {t("issueRelated.decompositions.planDocument")}
                  </Link>
                ) : null}
              </div>
