...
export function FileTree({
  ...
  const resolvedAriaLabel = ariaLabel ?? t("workspaceComp.fileTree.filesLabel");
    ...
    return (
      <div aria-label={resolvedAriaLabel} role="tree" className="p-3">
        <div className="rounded-md border border-dashed border-border px-4 py-8 text-center">
          <div className="text-sm font-medium">{empty?.title ?? t("workspaceComp.fileTree.emptyTitle")}</div>
          <div className="mt-1 text-xs text-muted-foreground">
            {empty?.description ?? t("workspaceComp.fileTree.emptyDescription")}
          </div>
        </div>
      </div>
    );
          ...
          >
            ...
            {node.kind === "dir" && (
              <button
                ...
                aria-label={expanded ? t("workspaceComp.fileTree.collapse", { name: node.name }) : t("workspaceComp.fileTree.expand", { name: node.name })}
              >
                {expanded ? (
