  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routinesComp.varsEditor.variables")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routinesComp.varsEditor.detectedFrom", { token: "{{name}}" })}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routinesComp.varsEditor.promptBeforeRun")}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routinesComp.varsEditor.label")}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routinesComp.varsEditor.type")}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("routinesComp.varsEditor.defaultValue")}</Label>
                  <label className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {t("routinesComp.varsEditor.required")}
                  </label>
                </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("routinesComp.varsEditor.noDefault")}</SelectItem>
                      <SelectItem value="true">{t("routinesComp.varsEditor.true")}</SelectItem>
                      <SelectItem value="false">{t("routinesComp.varsEditor.false")}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routinesComp.varsEditor.options")}</Label>
                      <Input
                        ...
                        placeholder={t("routinesComp.varsEditor.optionsPlaceholder")}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routinesComp.varsEditor.defaultOption")}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("routinesComp.varsEditor.noDefault")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("routinesComp.varsEditor.noDefault")}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? t("routinesComp.varsEditor.numberPlaceholder") : t("routinesComp.varsEditor.defaultValuePlaceholder")}
                  />
                )}
              </div>
            </div>
...
const BUILTIN_VARIABLE_DOCS: BuiltinVariableDoc[] = [
  {
    name: "date",
    example: "2026-04-28",
    descriptionKey: "routinesComp.varsEditor.dateDescription",
  },
  {
    name: "timestamp",
    example: "April 28, 2026 at 12:17 PM UTC",
    descriptionKey: "routinesComp.varsEditor.timestampDescription",
  },
];
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("routinesComp.varsEditor.hintText", { token: "{{variable_name}}" })}
        </span>
        <button
          type="button"
          ...
          aria-label={t("routinesComp.varsEditor.showVariableHelp")}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routinesComp.varsEditor.dialogTitle")}</DialogTitle>
            <DialogDescription>
              {t("routinesComp.varsEditor.dialogDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routinesComp.varsEditor.customVariables")}
              </h3>
              <p className="text-muted-foreground">
                <Trans
                  i18nKey="routinesComp.varsEditor.customVariablesBody"
                  values={{ token: "{{variable_name}}" }}
                  components={[
                    <code key="0" className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground" />,
                    <span key="1" className="font-medium text-foreground" />,
                  ]}
                />
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routinesComp.varsEditor.namingRule")}</li>
                <li>{t("routinesComp.varsEditor.typeRule")}</li>
                <li>{t("routinesComp.varsEditor.reuseRule")}</li>
              </ul>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routinesComp.varsEditor.builtinVariables")}
              </h3>
              <p className="text-muted-foreground">
                {t("routinesComp.varsEditor.builtinNote")}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("routinesComp.varsEditor.placeholderHeader")}</th>
                      <th className="px-3 py-2 font-medium">{t("routinesComp.varsEditor.exampleHeader")}</th>
                      <th className="px-3 py-2 font-medium">{t("routinesComp.varsEditor.descriptionHeader")}</th>
                    </tr>
                  </thead>
