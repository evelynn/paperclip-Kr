...
export function ReportsToPicker({
  ...
  const resolvedDisabledEmptyLabel = disabledEmptyLabel ?? t("agentConfig.reportsToPicker.disabledEmptyLabel");
  const resolvedChooseLabel = chooseLabel ?? t("agentConfig.reportsToPicker.chooseLabel");
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("agentConfig.reportsToPicker.unknownManager")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {`${t("agentConfig.reportsToPicker.reportsTo", { name: current.name })}${terminatedManager ? t("agentConfig.reportsToPicker.terminatedSuffix") : ""}`}
              </span>
            </>
          ) : (
            <>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        >
          {t("agentConfig.reportsToPicker.noManager")}
        </button>
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {t("agentConfig.reportsToPicker.current", { name: current.name })}
            </span>
          </div>
        )}
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("agentConfig.reportsToPicker.staleManagerNotice")}
          </div>
        )}
        {rows.map((a) => (
          <button
