...
export function ExecutionParticipantPicker({
  ...
  const participantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) return userLabel(value.slice("user:".length)) ?? t("agentConfig.participants.user");
    return value;
  };
  ...
  const label = stageType === "review" ? t("agentConfig.participants.reviewers") : t("agentConfig.participants.approvers");
  const searchPlaceholder = stageType === "review" ? t("agentConfig.participants.searchReviewers") : t("agentConfig.participants.searchApprovers");
  const noneLabel = stageType === "review" ? t("agentConfig.participants.noReviewers") : t("agentConfig.participants.noApprovers");
        ...
        <div className="max-h-48 overflow-y-auto overscroll-contain">
          ...
          {currentUserId && (
            ...
            >
              ...
              {t("agentConfig.participants.assignToMe")}
            </button>
          )}
          {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
            ...
            >
              <User className="h-3 w-3 shrink-0 text-muted-foreground" />
              {creatorUserLabel ?? t("agentConfig.participants.requester")}
            </button>
          )}
          {otherUserOptions
            .filter((option) => {
