        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("financeComp.providerQuota.tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("financeComp.providerQuota.tokensOut")}
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 && t("financeComp.providerQuota.apiRuns", { count: totalApiRuns })}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 && t("financeComp.providerQuota.subRuns", { count: totalSubRuns })}
                  {" "}{t("financeComp.providerQuota.runsSuffix")}
                </span>
              )}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            <QuotaBar
              label={t("financeComp.providerQuota.periodSpend")}
              percentUsed={budgetPct}
              leftLabel={formatCents(totalCostCents)}
              rightLabel={t("financeComp.providerQuota.pctOfAllocation", { percent: Math.round(budgetPct) })}
              showDeficitNotch={showDeficitNotch}
            />
            <QuotaBar
              label={t("financeComp.providerQuota.thisWeek")}
              percentUsed={weekPct}
              leftLabel={formatCents(weekSpendCents)}
              rightLabel={t("financeComp.providerQuota.perWeek", { amount: formatCents(Math.round(weeklyBudgetShare)) })}
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("financeComp.providerQuota.rollingWindows")}
              </p>
                    ...
                    <div key={w} className="space-y-1">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground font-mono flex-1">
                          {t("financeComp.providerQuota.tok", { tokens: formatTokens(tokens) })}
                        </span>
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("financeComp.providerQuota.subscription")}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span> {t("financeComp.providerQuota.runsSuffix")}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span> {t("financeComp.providerQuota.totalSuffix")}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("financeComp.providerQuota.tokensIn")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("financeComp.providerQuota.tokensOut")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("financeComp.providerQuota.subShare", { percent: Math.round(subSharePct) })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-3 shrink-0 tabular-nums text-xs">
                        <span className="text-muted-foreground">
                          {t("financeComp.providerQuota.tok", { tokens: formatTokens(rowTokens) })}
                        </span>
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("financeComp.providerQuota.tokenShareTitle", { percent: Math.round(tokenPct) })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("financeComp.providerQuota.costShareTitle", { percent: Math.round(costPct) })}
                      />
                    </div>
                  </div>
                );
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("financeComp.providerQuota.subscriptionQuota")}
                </p>
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("financeComp.providerQuota.percentUsed", { percent: qw.usedPercent })}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("financeComp.providerQuota.resets", { date: new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" }) })}
                            </p>
                          ) : null}
                        </div>
                      );
