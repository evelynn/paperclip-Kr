...
export function validateField(
  ...
  if (isRequired && (value === undefined || value === null || value === "")) {
    return t("onboardComp.jsonSchemaForm.validation.required");
  }
  ...
  if (type === "string" || type === "secret-ref") {
    ...
    if (schema.minLength != null && str.length < schema.minLength) {
      return t("onboardComp.jsonSchemaForm.validation.minLength", { min: schema.minLength });
    }
    if (schema.maxLength != null && str.length > schema.maxLength) {
      return t("onboardComp.jsonSchemaForm.validation.maxLength", { max: schema.maxLength });
    }
    if (schema.pattern) {
      ...
      if (schema.pattern.length <= MAX_PATTERN_LENGTH) {
        try {
          ...
          if (!re.test(str)) {
            return t("onboardComp.jsonSchemaForm.validation.pattern", { pattern: schema.pattern });
          }
        } catch {
  ...
  if (type === "number" || type === "integer") {
    ...
    if (isNaN(num)) return t("onboardComp.jsonSchemaForm.validation.validNumber");
    if (schema.minimum != null && num < schema.minimum) {
      return t("onboardComp.jsonSchemaForm.validation.minimum", { min: schema.minimum });
    }
    if (schema.maximum != null && num > schema.maximum) {
      return t("onboardComp.jsonSchemaForm.validation.maximum", { max: schema.maximum });
    }
    if (schema.exclusiveMinimum != null && num <= schema.exclusiveMinimum) {
      return t("onboardComp.jsonSchemaForm.validation.exclusiveMinimum", { min: schema.exclusiveMinimum });
    }
    if (schema.exclusiveMaximum != null && num >= schema.exclusiveMaximum) {
      return t("onboardComp.jsonSchemaForm.validation.exclusiveMaximum", { max: schema.exclusiveMaximum });
    }
    if (type === "integer" && !Number.isInteger(num)) {
      return t("onboardComp.jsonSchemaForm.validation.wholeNumber");
    }
    if (schema.multipleOf != null && num % schema.multipleOf !== 0) {
      return t("onboardComp.jsonSchemaForm.validation.multipleOf", { multiple: schema.multipleOf });
    }
  }
  ...
  if (type === "array") {
    ...
    if (schema.minItems != null && arr.length < schema.minItems) {
      return t("onboardComp.jsonSchemaForm.validation.minItems", { min: schema.minItems });
    }
    if (schema.maxItems != null && arr.length > schema.maxItems) {
      return t("onboardComp.jsonSchemaForm.validation.maxItems", { max: schema.maxItems });
    }
  }
    ...
    >
      ...
      >
        <SelectTrigger className="w-full">
          <SelectValue placeholder={t("onboardComp.jsonSchemaForm.selectOption")} />
        </SelectTrigger>
        <SelectContent>
          {showUnsetOption && (
            <SelectItem value={ENUM_UNSET_VALUE} textValue={t("onboardComp.jsonSchemaForm.none")}>
              <span className="text-muted-foreground">{t("onboardComp.jsonSchemaForm.none")}</span>
            </SelectItem>
          )}
          {options.map((option) => (
...
const SecretField = React.memo(({
    ...
    <div className="relative">
        ...
        <Textarea
          ...
          value={
            stringValue.length === 0
              ? ""
              : t("onboardComp.jsonSchemaForm.secret.hiddenSummary", { count: stringValue.length })
          }
          readOnly
          placeholder={String(defaultValue ?? "")}
          disabled={disabled}
      ...
      >
        ...
        <span className="sr-only">
          {isVisible ? t("onboardComp.jsonSchemaForm.secret.hide") : t("onboardComp.jsonSchemaForm.secret.show")}
        </span>
      </Button>
    </div>
  ) : (
  ...
  return (
    <FieldWrapper
      ...
      description={
        description ||
        t("onboardComp.jsonSchemaForm.secret.defaultDescription")
      }
      required={isRequired}
      error={error}
      disabled={disabled}
    >
      <div className="space-y-2">
        <SecretBindingPicker
          ...
          label=""
          placeholder={t("onboardComp.jsonSchemaForm.secret.pickerPlaceholder")}
          allowVersionSelector={false}
          emptyHint={t("onboardComp.jsonSchemaForm.secret.emptyHint")}
          disabled={disabled}
        />
        {!isBoundToSecret ? (
          showRawInput ? (
            <div className="space-y-1">
              ...
              {!hasRawValue ? (
                <button
                  ...
                  disabled={disabled}
                >
                  {t("onboardComp.jsonSchemaForm.secret.hideRawInput")}
                </button>
              ) : null}
            </div>
          ) : (
            <button
              ...
              disabled={disabled}
            >
              {t("onboardComp.jsonSchemaForm.secret.pasteRawValue")}
            </button>
          )
        ) : null}
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {isComplex ? t("onboardComp.jsonSchemaForm.array.addItem") : t("onboardComp.jsonSchemaForm.array.add")}
        </Button>
      </div>
          ...
          >
            <div className="flex-1">
              <div className="mb-2 text-xs font-medium text-muted-foreground">
                {t("onboardComp.jsonSchemaForm.array.item", { index: index + 1 })}
              </div>
              <FormField
            ...
            >
              ...
              <span className="sr-only">{t("onboardComp.jsonSchemaForm.array.removeItem")}</span>
            </Button>
          </div>
        ))}
        {items.length === 0 && (
          <div className="rounded-lg border border-dashed p-4 text-center text-xs text-muted-foreground">
            {t("onboardComp.jsonSchemaForm.array.noItems")}
          </div>
        )}
      </div>
      {error && (
...
export function JsonSchemaForm({
  ...
  const { essentials, advancedGroups, advancedKeys } = useMemo(() => {
    ...
    const DEFAULT_GROUP = t("onboardComp.jsonSchemaForm.moreOptionsGroup");
  ...
  if (Object.keys(properties).length === 0) {
    return (
      ...
      >
        {t("onboardComp.jsonSchemaForm.noOptions")}
      </div>
    );
  }
    ...
    <div className={cn("space-y-6", className)}>
      ...
      {hasAdvanced && (
        <div className="space-y-3 rounded-lg border border-dashed">
          ...
          >
            <span className="text-sm font-medium">{t("onboardComp.jsonSchemaForm.advancedOptions")}</span>
            {isAdvancedOpen ? (
              <ChevronDown className="h-4 w-4 text-muted-foreground" />
            ) : (
              <ChevronRight className="h-4 w-4 text-muted-foreground" />
