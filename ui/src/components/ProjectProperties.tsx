...
const PROJECT_STATUSES = [
  { value: "backlog", labelKey: "projectsComp.status.backlog" },
  { value: "planned", labelKey: "projectsComp.status.planned" },
  { value: "in_progress", labelKey: "projectsComp.status.inProgress" },
  { value: "completed", labelKey: "projectsComp.status.completed" },
  { value: "cancelled", labelKey: "projectsComp.status.cancelled" },
];
...
function ProjectStatusPicker({ status, onChange }: { status: string; onChange: (status: string) => void }) {
  ...
  const activeStatus = PROJECT_STATUSES.find((s) => s.value === status);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {activeStatus ? t(activeStatus.labelKey) : status.replace("_", " ")}
        </button>
      </PopoverTrigger>
  ...
  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("projectsComp.archive.archiveDescription")
          : t("projectsComp.archive.unarchiveDescription")}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("projectsComp.archive.archiving") : t("projectsComp.archive.unarchiving")}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {isArchive
              ? t("projectsComp.archive.confirmArchive", { name: project.name })
              : t("projectsComp.archive.confirmUnarchive", { name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
          ...
          >
            {t("projectsComp.archive.confirm")}
          </Button>
          <Button
            size="sm"
            variant="outline"
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{t("projectsComp.archive.archiveProject")}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{t("projectsComp.archive.unarchiveProject")}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projectsComp.codebase.localFolderAbsoluteError"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectsComp.codebase.repoUrlInvalidError"));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("projectsComp.codebase.clearLocalFolderConfirm")
        : t("projectsComp.codebase.deleteLocalFolderConfirm"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("projectsComp.codebase.clearRepoConfirm")
        : t("projectsComp.codebase.deleteRepoConfirm"),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("projectsComp.fields.name")} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={t("projectsComp.fields.namePlaceholder")}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectsComp.fields.description")} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("projectsComp.fields.descriptionPlaceholder")}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("projectsComp.fields.noDescription")}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectsComp.fields.status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("projectsComp.fields.lead")}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("projectsComp.fields.goals")} state={fieldState("goals")} />}
          alignStart
                ...
                >
                  ...
                  {(onUpdate || onFieldUpdate) && (
                    <button
                      ...
                      aria-label={t("projectsComp.goals.removeGoalAria", { title: goal.title })}
                    >
                      <X className="h-3 w-3" />
                    </button>
                  )}
            ...
            <Popover open={goalOpen} onOpenChange={setGoalOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {t("projectsComp.goals.goal")}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {t("projectsComp.goals.allGoalsLinked")}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectsComp.fields.env")} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("projectsComp.env.help")}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectsComp.fields.created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectsComp.fields.updated")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("projectsComp.fields.targetDate")} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("projectsComp.codebase.title")}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={t("projectsComp.codebase.helpAria")}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("projectsComp.codebase.helpTooltip")}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("projectsComp.codebase.repo")}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {t("projectsComp.codebase.changeRepo")}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={t("projectsComp.codebase.clearRepoAria")}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{t("projectsComp.codebase.notSet")}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
                  ...
                  >
                    {t("projectsComp.codebase.setRepo")}
                  </Button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("projectsComp.codebase.localFolder")}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("projectsComp.codebase.managedFolder")}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("projectsComp.codebase.changeLocalFolder") : t("projectsComp.codebase.setLocalFolder")}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      ...
                      aria-label={t("projectsComp.codebase.clearLocalFolderAria")}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("projectsComp.codebase.legacyWorkspaces")}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("projectsComp.codebase.noUrl")
                        )}
                      </div>
                    </div>
          ...
          {workspaceMode === "local" && (
            <div className="space-y-1.5 rounded-md border border-border p-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {t("projectsComp.codebase.cancel")}
                </Button>
              </div>
            </div>
          )}
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectsComp.codebase.saveWorkspaceError")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectsComp.codebase.deleteWorkspaceError")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectsComp.codebase.updateWorkspaceError")}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("projectsComp.execution.title")}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={t("projectsComp.execution.helpAria")}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {t("projectsComp.execution.helpTooltip")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("projectsComp.execution.enableIsolated")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("projectsComp.execution.enableIsolatedDescription")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("projectsComp.execution.enabled") : t("projectsComp.execution.disabled")}
                    </span>
                  )}
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectsComp.execution.defaultIsolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectsComp.execution.defaultIsolatedDescription")}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("projectsComp.execution.hideAdvanced")
                          : t("projectsComp.execution.showAdvanced")}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          <Trans
                            i18nKey="projectsComp.execution.hostManaged"
                          ...
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{t("projectsComp.execution.environment")}</span>
                            ...
                            >
                              <option value="">{t("projectsComp.execution.noEnvironment")}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectsComp.execution.baseRef")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectsComp.execution.branchTemplate")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectsComp.execution.worktreeParentDir")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectsComp.execution.provisionCommand")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectsComp.execution.teardownCommand")}</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("projectsComp.execution.provisionTeardownHelp")}
                        </p>
                      </div>
                    ) : null}
                  </div>
