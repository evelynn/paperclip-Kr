...
export function ActiveAgentsPanel({
  ...
  const resolvedTitle = title ?? t("agentConfig.activeAgents.defaultTitle");
  const resolvedEmptyMessage = emptyMessage ?? t("agentConfig.activeAgents.emptyMessage");
      ...
      {showMoreLink && hiddenRunCount > 0 && (
        <div className="mt-3 flex justify-end text-xs text-muted-foreground">
          <Link to="/dashboard/live" className="hover:text-foreground hover:underline">
            {t(hiddenRunCount === 1 ? "agentConfig.activeAgents.moreRunsOne" : "agentConfig.activeAgents.moreRunsOther", { count: hiddenRunCount })}
          </Link>
        </div>
      )}
    </div>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("agentConfig.activeAgents.liveNow") : run.finishedAt ? t("agentConfig.activeAgents.finished", { time: relativeTime(run.finishedAt) }) : t("agentConfig.activeAgents.started", { time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
