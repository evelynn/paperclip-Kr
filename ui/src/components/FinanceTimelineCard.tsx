...
export function FinanceTimelineCard({
  ...
  const resolvedEmptyMessage = emptyMessage ?? t("financeComp.financeTimeline.defaultEmpty");
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("financeComp.financeTimeline.title")}</CardTitle>
        <CardDescription>{t("financeComp.financeTimeline.description")}</CardDescription>
      </CardHeader>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("financeComp.financeTimeline.invoice", { id: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("financeComp.financeTimeline.region", { region: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("financeComp.financeTimeline.tier", { tier: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-[11px] uppercase tracking-[0.12em] text-amber-600">{t("financeComp.financeTimeline.estimated")}</div> : null}
                </div>
              </div>
            </div>
          ))
