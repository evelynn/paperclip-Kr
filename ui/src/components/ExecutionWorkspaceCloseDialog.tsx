...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed"
    ? t("workspaceComp.closeDialog.retryTitle")
    : t("workspaceComp.closeDialog.closeTitle");
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed"
          ? t("workspaceComp.closeDialog.toast.retried")
          : t("workspaceComp.closeDialog.toast.closed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("workspaceComp.closeDialog.toast.failed"),
        body: error instanceof Error ? error.message : t("workspaceComp.closeDialog.toast.unknownError"),
        tone: "error",
      });
    },
  });
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            <Trans
              i18nKey="workspaceComp.closeDialog.description"
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("workspaceComp.closeDialog.checking")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("workspaceComp.closeDialog.inspectError")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("workspaceComp.closeDialog.stateBlocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("workspaceComp.closeDialog.stateWarnings")
                    : t("workspaceComp.closeDialog.stateReady")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("workspaceComp.closeDialog.sharedNote")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("workspaceComp.closeDialog.ownCheckoutNote")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("workspaceComp.closeDialog.primaryWorkspaceNote")
                      : t("workspaceComp.closeDialog.disposableNote")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.blockingTasks")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.blockingReasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.closeDialog.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("workspaceComp.closeDialog.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.closeDialog.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("workspaceComp.closeDialog.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.closeDialog.mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("workspaceComp.closeDialog.unknown") : readiness.git.isMergedIntoBase ? t("workspaceComp.closeDialog.yes") : t("workspaceComp.closeDialog.no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.closeDialog.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.closeDialog.dirtyTracked")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.closeDialog.untrackedFiles")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.otherLinkedTasks")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.attachedRuntimeServices")}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? t("workspaceComp.closeDialog.noServiceDetails")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("workspaceComp.closeDialog.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("workspaceComp.closeDialog.cleanupFailedNote")}
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
                {t("workspaceComp.closeDialog.alreadyArchived")}
              </div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {t("workspaceComp.closeDialog.repoRoot")} <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{t("workspaceComp.closeDialog.workspacePath")} <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("workspaceComp.closeDialog.lastChecked", { time: formatDateTime(new Date()) })}
            </div>
          </div>
        ) : null}
        ...
        <DialogFooter>
          ...
          >
            {t("workspaceComp.closeDialog.cancel")}
          </Button>
          <Button
