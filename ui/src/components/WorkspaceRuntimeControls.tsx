    ...
    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("workspaceComp.runtimeControls.actions.run")
          : action === "start"
            ? t("workspaceComp.runtimeControls.actions.start")
            : action === "stop"
              ? t("workspaceComp.runtimeControls.actions.stop")
              : t("workspaceComp.runtimeControls.actions.restart");
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("workspaceComp.runtimeControls.port", { port: item.port })}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  isPending = false,
  pendingRequest = null,
  serviceEmptyMessage: serviceEmptyMessageProp,
  jobEmptyMessage: jobEmptyMessageProp,
  emptyMessage,
  disabledHint = null,
  onAction,
  className,
  ...
  const { t } = useTranslation();
  const serviceEmptyMessage = serviceEmptyMessageProp ?? t("workspaceComp.runtimeControls.serviceEmptyMessage");
  const jobEmptyMessage = jobEmptyMessageProp ?? t("workspaceComp.runtimeControls.jobEmptyMessage");
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceComp.runtimeControls.header")}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0
                ? t(
                    runningCount === 1
                      ? "workspaceComp.runtimeControls.servicesRunningOne"
                      : "workspaceComp.runtimeControls.servicesRunningOther",
                    { count: runningCount },
                  )
                : t("workspaceComp.runtimeControls.noServicesRunning")}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? t(
                    resolvedSections.jobs.length === 1
                      ? "workspaceComp.runtimeControls.jobsAvailableOne"
                      : "workspaceComp.runtimeControls.jobsAvailableOther",
                    { count: resolvedSections.jobs.length },
                  )
                : t("workspaceComp.runtimeControls.commandsIndependent")}
            </span>
          </div>
      ...
      <CommandSection
        title={t("workspaceComp.runtimeControls.services.title")}
        description={t("workspaceComp.runtimeControls.services.description")}
      ...
      <CommandSection
        title={t("workspaceComp.runtimeControls.jobs.title")}
        description={t("workspaceComp.runtimeControls.jobs.description")}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaceComp.runtimeControls.untracked.title")}
          description={t("workspaceComp.runtimeControls.untracked.description")}
