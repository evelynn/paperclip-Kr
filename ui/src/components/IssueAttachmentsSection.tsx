...
function AttachmentActions({
  ...
  return (
    <div className="flex shrink-0 items-center gap-1">
      <Button asChild variant="ghost" size="icon-sm" title={t("issueDocs.attachments.openInNewTab")}>
        <a href={attachmentOpenPath(attachment)} target="_blank" rel="noreferrer" aria-label={t("issueDocs.attachments.openAriaLabel", { name: filename })}>
          <ExternalLink className="h-4 w-4" />
        </a>
      </Button>
      <Button asChild variant="ghost" size="icon-sm" title={t("issueDocs.attachments.download")}>
        <a href={attachmentDownloadPath(attachment)} aria-label={t("issueDocs.attachments.downloadAriaLabel", { name: filename })}>
      ...
      <Button
        variant="ghost"
        size="icon-sm"
        title={t("issueDocs.attachments.deleteAttachment")}
...
function AttachmentMeta({ attachment }: { attachment: IssueAttachment }) {
  ...
  return (
    <p className="mt-0.5 text-[11px] text-muted-foreground">
      {t("issueDocs.attachments.meta", { contentType: attachment.contentType, size: formatBytes(attachment.byteSize) })}
    </p>
  );
}
  ...
  return (
    <div id={`attachment-${attachment.id}`} className="scroll-mt-20 rounded-lg border border-border p-3">
      ...
      <div className="mt-3 rounded-md hover:bg-accent/10">
        {isLoading ? (
          <p className="px-1 py-2 text-xs text-muted-foreground">{t("issueDocs.attachments.loadingPreview")}</p>
        ) : error ? (
          <p className="px-1 py-2 text-xs text-destructive">{t("issueDocs.attachments.markdownPreviewFailed")}</p>
        ) : (
          <FoldCurtain>
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-muted-foreground">{t("issueDocs.attachments.heading")}</h3>
          ...
          {imageAttachments.map((attachment) => (
            ...
            >
              <img
                ...
                alt={attachment.originalFilename ?? t("issueDocs.attachments.imageFallbackAlt")}
              ...
              {confirmDeleteId === attachment.id ? (
                ...
                >
                  <p className="text-xs font-medium text-white">{t("issueDocs.attachments.deletePrompt")}</p>
      ...
      {confirmDeleteId && !imageAttachments.some((attachment) => attachment.id === confirmDeleteId) ? (
        <div className="flex items-center justify-between gap-3 rounded-md border border-destructive/20 bg-destructive/5 px-4 py-3">
          <p className="text-sm font-medium text-destructive">{t("issueDocs.attachments.deleteConfirm")}</p>
          <div className="flex shrink-0 items-center gap-2">
            <Button variant="ghost" size="sm" onClick={() => setConfirmDeleteId(null)} disabled={deletePending}>
              {t("issueDocs.attachments.cancel")}
            </Button>
            <Button variant="destructive" size="sm" onClick={() => confirmDelete(confirmDeleteId)} disabled={deletePending}>
              {deletePending ? t("issueDocs.attachments.deleting") : t("issueDocs.attachments.delete")}
            </Button>
          </div>
        </div>
      ) : null}
