...
function windowLabelKey(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime"
    ? "financeComp.budgetPolicy.windowLifetime"
    : "financeComp.budgetPolicy.windowMonthly";
}
...
export function BudgetPolicyCard({
  ...
  const statusLabel = summary.paused
    ? t("financeComp.budgetPolicy.statusPaused")
    : summary.status === "warning"
      ? t("financeComp.budgetPolicy.statusWarning")
      : summary.status === "hard_stop"
        ? t("financeComp.budgetPolicy.statusHardStop")
        : t("financeComp.budgetPolicy.statusHealthy");

  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("financeComp.budgetPolicy.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("financeComp.budgetPolicy.pctOfLimit", { percent: summary.utilizationPercent }) : t("financeComp.budgetPolicy.noCap")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("financeComp.budgetPolicy.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("financeComp.budgetPolicy.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("financeComp.budgetPolicy.softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? t("financeComp.budgetPolicy.pauseSuffix", { reason: summary.pauseReason }) : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("financeComp.budgetPolicy.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("financeComp.budgetPolicy.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("financeComp.budgetPolicy.pausedProject")
          : t("financeComp.budgetPolicy.pausedScope")}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("financeComp.budgetPolicy.budgetUsd")}
        </label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving ? t("financeComp.budgetPolicy.saving") : summary.amount > 0 ? t("financeComp.budgetPolicy.updateBudget") : t("financeComp.budgetPolicy.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">{t(windowLabelKey(summary.windowKind))}</div>
          </div>
          <div
            className={cn(
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("financeComp.budgetPolicy.invalidAmount")}</p>
        ) : null}
      </div>
    );
  }
      ...
      <CardHeader className={cn("gap-3", compact ? "px-4 pt-4 pb-2" : "px-5 pt-5 pb-3")}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">{t(windowLabelKey(summary.windowKind))}</CardDescription>
          </div>
