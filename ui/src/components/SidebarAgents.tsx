...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("nav.agents.resume") : t("nav.agents.pause");
  ...
  const pauseResumeDisabledLabel = disabled
    ? t("nav.agents.updating")
    : isBudgetPaused
      ? t("nav.agents.budgetPaused")
      : isPaused && hasInvalidOrgChain
        ? t("nav.agents.invalidOrgChain")
      : pauseResumeLabel;
  ...
  const link = (
    ...
    >
      ...
      {!rail && hasInvalidOrgChain ? (
        <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-amber-500" aria-label={t("nav.agents.invalidReportingChain")} />
      ) : null}
      {!rail && (agent.pauseReason === "budget" || runCount > 0) && (
        <span className="ml-auto flex items-center gap-1.5 shrink-0">
          {agent.pauseReason === "budget" ? (
            <BudgetSidebarMarker title={t("nav.agents.pausedByBudget")} />
          ) : null}
          {runCount > 0 ? (
          ...
          {runCount > 0 ? (
            <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
              {t("nav.agents.liveCount", { count: runCount })}
            </span>
          ) : null}
        </span>
      )}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("nav.agents.openActions", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-44">
          ...
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("nav.agents.pausedByBudgetLimits") : undefined}
          >
          ...
          >
            ...
            <span>{leaving ? t("nav.agents.leaving") : t("nav.agents.leave")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      )}
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const agentSortChoices = useMemo<SidebarSectionRadioChoice[]>(
    () => [
      { value: "top", label: t("nav.agents.sortTop") },
      { value: "alphabetical", label: t("nav.agents.sortAlphabetical") },
      { value: "recent", label: t("nav.agents.sortRecent") },
    ],
    [t],
  );
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? t("nav.agents.toastPaused") : t("nav.agents.toastResumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("nav.agents.toastPauseFailed") : t("nav.agents.toastResumeFailed"),
  ...
  return (
    <SidebarSection
      label={t("nav.agents.section")}
      collapsible={{ open, onOpenChange: setOpen }}
      headerAction={{
        ariaLabel: t("nav.agents.newAgent"),
        icon: Plus,
        onClick: openNewAgent,
      }}
      menu={{
        ariaLabel: t("nav.agents.sectionActions"),
        actions: [
          { type: "item", label: t("nav.agents.browse"), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("nav.agents.sort"),
      ...
      {showSeeAllLink && (() => {
        const seeAllLink = (
          <Link
            to="/agents/all"
            state={SIDEBAR_SCROLL_RESET_STATE}
            aria-label={rail ? t("nav.agents.seeAll") : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>{t("nav.agents.seeAll")}</span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            <TooltipTrigger asChild>{seeAllLink}</TooltipTrigger>
            <TooltipContent side="right">{t("nav.agents.seeAll")}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );
