...
export function RoutineTriggerCard({
  ...
  return (
    <form
      aria-label={t("routinesComp.triggerCard.triggerAria", { label: trigger.label ?? trigger.kind })}
    ...
    >
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {trigger.kind === "schedule" && trigger.nextRunAt
              ? t("routinesComp.triggerCard.next", { time: new Date(trigger.nextRunAt).toLocaleString() })
              : trigger.kind === "webhook"
                ? t("routinesComp.triggerCard.webhook")
                : t("routinesComp.triggerCard.api")}
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("routinesComp.triggerCard.label")}</Label>
          <Input
            value={draft.label}
            disabled={disabled}
        ...
        {trigger.kind === "schedule" && (
          <div className="space-y-1.5 md:col-span-2">
            <Label className="text-xs">{t("routinesComp.triggerCard.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routinesComp.triggerCard.signingMode")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routinesComp.triggerCard.replayWindow")}</Label>
                <Input
      ...
      {!disabled && (
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            {t("routinesComp.triggerCard.delete")}
          </Button>
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("routinesComp.triggerCard.rotateSecret")}
            </Button>
          )}
          <Button
            variant="outline"
          ...
          >
            ...
            {t("routinesComp.triggerCard.saveTrigger")}
          </Button>
        </div>
      )}
    </form>
