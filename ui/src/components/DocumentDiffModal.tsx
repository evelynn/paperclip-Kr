...
function getRevisionLabel(revision: DocumentRevision, t: TFunction) {
  const actor = revision.createdByUserId
    ? t("issueDocs.documents.actorBoard")
    : revision.createdByAgentId
      ? t("issueDocs.documents.actorAgent")
      : t("issueDocs.documents.actorSystem");
  return t("issueDocs.diff.revisionLabel", {
      ...
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>
              {t("issueDocs.diff.title")} — <span className="font-mono text-sm">{documentKey}</span>
            </DialogTitle>
          </DialogHeader>

          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-red-400">{t("issueDocs.diff.old")}</span>
              <Select
              ...
              >
                <SelectTrigger className="h-7 w-60 text-xs border-border/60">
                  <SelectValue placeholder={t("issueDocs.diff.selectRevisionPlaceholder")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-green-500/30 bg-green-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-green-400">{t("issueDocs.diff.new")}</span>
              <Select
        ...
        <div className="overflow-auto flex-1 rounded-md border border-border text-xs">
          {!revisions ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("issueDocs.diff.loadingRevisions")}</div>
          ) : !leftRevision || !rightRevision ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("issueDocs.diff.selectTwoRevisions")}</div>
          ) : leftRevision.id === rightRevision.id ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("issueDocs.diff.sameRevision")}</div>
          ) : (
            <div className="font-mono text-[12px] leading-6">
              <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
                <span>{t("issueDocs.diff.old")}</span>
                <span>{t("issueDocs.diff.new")}</span>
                <span />
                <span>{t("issueDocs.diff.content")}</span>
              </div>
