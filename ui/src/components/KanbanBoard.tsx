  ...
  if (collapsed) {
    return (
      <div
        ...
        title={`${issueStatusLabel(t, status)}: ${issues.length}`}
      >
      ...
      >
        ...
        {hiddenCount > 0 ? (
          <button
            ...
            onClick={onShowMore}
          >
            {t("issues.board.showMore", { count: nextRevealCount })}
          </button>
        ) : null}
        {issues.length > 0 && (hiddenCount > 0 || issues.length >= visibleCount) ? (
          <p className="px-1 pt-1 text-[11px] text-muted-foreground">
            {t("issues.board.showingOf", { shown: visibleIssues.length, total: issues.length })}
          </p>
        ) : null}
      </div>
    </div>
        ...
        <div className={`flex items-start gap-1.5 ${compact ? "mb-1" : "mb-1.5"}`}>
          ...
          {isLive && (
            <span className="inline-flex shrink-0 items-center gap-1 text-[10px] font-medium text-blue-600 dark:text-blue-400">
              ...
              {compact ? t("issues.board.live") : null}
            </span>
          )}
