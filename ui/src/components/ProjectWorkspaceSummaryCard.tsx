            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
                {summary.kind === "execution_workspace"
                  ? t("projectsComp.workspaceCard.executionWorkspace")
                  : t("projectsComp.workspaceCard.projectWorkspace")}
              </span>
              <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                {t("projectsComp.workspaceCard.updated", { time: timeAgo(summary.lastUpdatedAt) })}
              </span>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("projectsComp.workspaceCard.servicesCount", {
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("projectsComp.workspaceCard.stopServices") : t("projectsComp.workspaceCard.startServices")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("projectsComp.workspaceCard.retryClose") : t("projectsComp.workspaceCard.closeWorkspace")}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectsComp.workspaceCard.branch")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectsComp.workspaceCard.branchCopied")}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("projectsComp.workspaceCard.copyBranch")}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectsComp.workspaceCard.path")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectsComp.workspaceCard.pathCopied")}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("projectsComp.workspaceCard.copyPath")}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("projectsComp.workspaceCard.pathCopied")}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectsComp.workspaceCard.service")}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {t("projectsComp.workspaceCard.linkedTasks")}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("projectsComp.workspaceCard.moreCount", { count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>
