...
function incidentStateKey(incident: BudgetIncident) {
  if (incident.status === "resolved") return "financeComp.budgetIncident.stateResolved";
  if (incident.status === "dismissed") return "financeComp.budgetIncident.stateDismissed";
  if (incident.approvalStatus === "revision_requested") return "financeComp.budgetIncident.stateEscalated";
  if (incident.approvalStatus === "pending") return "financeComp.budgetIncident.statePending";
  return "financeComp.budgetIncident.stateOpen";
}
...
export function BudgetIncidentCard({
  ...
  const stateLabel = t(incidentStateKey(incident));
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
                {t("financeComp.budgetIncident.hardStop", { scope: incident.scopeType })}
              </div>
            ...
            <CardDescription className="mt-1 text-red-100/70">
              {t("financeComp.budgetIncident.spendingReached", {
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-0">
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("financeComp.budgetIncident.pausedProject")
              : t("financeComp.budgetIncident.pausedScope")}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            {t("financeComp.budgetIncident.newBudgetUsd")}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? t("financeComp.budgetIncident.applying") : t("financeComp.budgetIncident.raiseResume")}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              {t("financeComp.budgetIncident.mustExceed")}
            </p>
          ) : null}
        </div>
        ...
        <div className="flex justify-end">
          <Button variant="ghost" className="text-muted-foreground" disabled={isMutating} onClick={onKeepPaused}>
            {t("financeComp.budgetIncident.keepPaused")}
          </Button>
        </div>
      </CardContent>
    </Card>
