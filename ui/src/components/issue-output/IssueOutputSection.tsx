...
export function IssueOutputSection({ workProducts, resolveCreatorName }: IssueOutputSectionProps) {
  ...
  return (
    <section className="space-y-3" aria-label={t("tailComp.issueOutput.sectionAriaLabel")}>
      <div className="flex items-center gap-2">
        <Play className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
        <h3 className="text-sm font-medium text-muted-foreground">{t("tailComp.issueOutput.heading")}</h3>
      ...
      {rest.length > 0 && (
        <div className="space-y-2">
          <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t("tailComp.issueOutput.alsoProduced")}</p>
          {rest.map((item) => (
