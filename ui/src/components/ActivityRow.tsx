...
export function ActivityRow({ event, agentMap, userProfileMap, entityNameMap, entityTitleMap, className }: ActivityRowProps) {
  ...
  const actorName = actor?.name ?? (event.actorType === "system" ? t("feedComp.actor.system") : userProfile?.label ?? (event.actorType === "user" ? t("feedComp.actor.board") : event.actorId || t("feedComp.actor.unknown")));
