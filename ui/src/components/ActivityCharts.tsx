...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("financeComp.activity.noRunsYet")}</p>;
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t(total === 1 ? "financeComp.activity.runTooltipOne" : "financeComp.activity.runTooltipOther", { day, count: total })}>
              {total > 0 ? (
...
const priorityLabelKeys: Record<string, string> = {
  critical: "financeComp.activity.priorityCritical",
  high: "financeComp.activity.priorityHigh",
  medium: "financeComp.activity.priorityMedium",
  low: "financeComp.activity.priorityLow",
};
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("financeComp.activity.noTasks")}</p>;
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t(total === 1 ? "financeComp.activity.issueTooltipOne" : "financeComp.activity.issueTooltipOther", { day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(priorityLabelKeys[p]) }))} />
    </div>
  );
}
...
const statusLabelKeys: Record<string, string> = {
  todo: "financeComp.activity.statusTodo",
  in_progress: "financeComp.activity.statusInProgress",
  in_review: "financeComp.activity.statusInReview",
  done: "financeComp.activity.statusDone",
  blocked: "financeComp.activity.statusBlocked",
  cancelled: "financeComp.activity.statusCancelled",
  backlog: "financeComp.activity.statusBacklog",
};
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("financeComp.activity.successTooltip", { day, percent: entry.total > 0 ? Math.round(rate * 100) : 0, succeeded: entry.succeeded, total: entry.total })}>
              {entry.total > 0 ? (
