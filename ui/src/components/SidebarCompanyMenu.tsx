    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={t("nav.company.reorder", { name: company.name })}
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedCompany ? t("nav.company.openSwitcherNamed", { name: selectedCompany.name }) : t("nav.company.openSwitcher")}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <span className={cn("truncate text-sm font-bold text-foreground", rail && SIDEBAR_RAIL_HIDDEN_LABEL)}>
              {selectedCompany?.name ?? t("nav.company.selectWorkspace")}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          ...
          >
            {isEditingOrder ? t("nav.company.done") : t("nav.company.edit")}
          </button>
        </div>
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{t("nav.company.noWorkspaces")}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{conferenceRoomChatEnabled ? t("nav.company.createTeam") : t("nav.company.addCompany")}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedCompany ? t("nav.company.invitePeopleNamed", { name: selectedCompany.name }) : t("nav.company.invitePeople")}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{t("nav.company.companySettings")}</span>
          </Link>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending ? t("nav.company.signingOut") : t("nav.company.signOut")}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>
