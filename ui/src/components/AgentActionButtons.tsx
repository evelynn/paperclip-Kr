...
export function RunButton({
  ...
  return (
    <Button variant="outline" size={size} onClick={onClick} disabled={disabled}>
      ...
      <span className="hidden sm:inline">{label ?? t("chatThread.runNow")}</span>
    </Button>
  );
}
...
export function PauseResumeButton({
  ...
  if (isPaused) {
    return (
      <Button variant="outline" size={size} onClick={onResume} disabled={disabled}>
        <Play className="h-3.5 w-3.5 sm:mr-1" />
        <span className="hidden sm:inline">{t("chatThread.resume")}</span>
      </Button>
    );
  }
  ...
  return (
    <Button variant="outline" size={size} onClick={onPause} disabled={disabled}>
      ...
      <span className="hidden sm:inline">{t("chatThread.pause")}</span>
    </Button>
  );
}
...
export function ClearErrorButton({
  ...
  return (
    <Button
      ...
      aria-label={t("chatThread.clearErrorAria")}
    >
      <CheckCircle2 className="h-3.5 w-3.5 sm:mr-1" />
      <span className="hidden sm:inline">{t("chatThread.clearError")}</span>
    </Button>
  );
}
...
export function AgentActionButtons({
  ...
  const { t } = useTranslation();
  const assignLabel = assignLabelProp ?? t("chatThread.assignTask");
  const runLabel = runLabelProp ?? t("chatThread.runNow");
  ...
  const reportError = useCallback(
    (message: string) => {
      if (onActionError) {
        ...
        pushToast({ title: t("chatThread.actionFailed"), body: message, tone: "error" });
      }
    },
  ...
  const agentAction = useMutation({
    ...
    onError: (err) => {
      reportError(err instanceof Error ? err.message : t("chatThread.actionFailed"));
    },
  });
  ...
  const duplicateAgent = useMutation({
    mutationFn: async () => {
      if (!resolvedCompanyId) {
        throw new Error(t("chatThread.agentNotReadyToDuplicate"));
      }
    ...
    onSuccess: async (createdAgent) => {
      ...
      pushToast({ title: t("chatThread.agentDuplicated"), body: createdAgent.name, tone: "success" });
      navigate(`/agents/${agentRouteRef(createdAgent)}/dashboard`);
    },
    onError: (err) => {
      const message = err instanceof Error ? err.message : t("chatThread.failedToDuplicateAgent");
      onActionError?.(message);
      pushToast({ title: t("chatThread.couldNotDuplicateAgent"), body: message, tone: "error" });
    },
  });
  ...
  const handleDuplicateAgent = useCallback(() => {
    ...
    const confirmed = window.confirm(t("chatThread.confirmDuplicateAgent", { name: agent.name, nextName }));
    setMoreOpen(false);
    if (!confirmed) return;
  ...
  const resetTaskSession = useMutation({
    ...
    onError: (err) => {
      reportError(err instanceof Error ? err.message : t("chatThread.failedToResetSession"));
    },
  });
    ...
    <div className={className ?? "flex items-center gap-1 sm:gap-2 shrink-0"}>
      ...
      <Popover open={moreOpen} onOpenChange={setMoreOpen}>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="icon-xs" aria-label={t("chatThread.openActionsForAgent", { name: agent.name })}>
