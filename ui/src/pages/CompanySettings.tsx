...
export function CompanySettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.common.settings") }
    ]);
  ...
  if (!selectedCompany) {
    return (
      <div className="text-sm text-muted-foreground">
        {t("settings.company.noSelection")}
      </div>
    );
  }
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <Field label={t("settings.company.companyName")} hint={t("settings.company.companyNameHint")}>
            <input
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {logoUrl && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {clearLogoMutation.isPending ? t("settings.company.removing") : t("settings.company.removeLogo")}
                      </Button>
                    </div>
                  )}
              ...
              <Field
                label={t("settings.company.attachmentLimit")}
              ...
              >
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-muted-foreground">{t("settings.company.mib")}</span>
                  </div>
                  {!attachmentMaxValid && (
                    <span className="text-xs text-destructive">
                      {t("settings.company.attachmentRangeError", { max: MAX_COMPANY_ATTACHMENT_MAX_MIB })}
                    </span>
                  )}
                </div>
              </Field>
      ...
      <div className="space-y-4" data-testid="company-settings-team-section">
        ...
        <div className="rounded-md border border-border px-4 py-3">
          <ToggleField
            label={t("settings.company.requireApproval")}
      ...
      <div className="space-y-4">
        <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
          {t("settings.company.packages")}
        </div>
        <div className="rounded-md border border-border px-4 py-4">
          <p className="text-sm text-muted-foreground">
            <Trans
              i18nKey="settings.company.packagesMoved"
              components={[<a href="/org" className="underline hover:text-foreground" />]}
            />
          </p>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const confirmed = window.confirm(
                  t("settings.company.confirmArchive", { name: selectedCompany.name })
                );
                if (!confirmed) return;
                const nextCompanyId =
                  companies.find(
            ...
            {archiveMutation.isError && (
              <span className="text-xs text-destructive">
                {archiveMutation.error instanceof Error
                  ...
                  : t("settings.company.archiveFailed")}
              </span>
            )}
          </div>
        </div>
