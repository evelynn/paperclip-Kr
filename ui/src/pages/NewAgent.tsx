...
export function NewAgent() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("agentsPage.list.breadcrumb"), href: "/agents" },
      { label: t("agentsPage.new.heading") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("agentsPage.new.createFailed"));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      if (!isValidOpenCodeModelId(configValues.model)) {
        setFormError(t("agentsPage.new.openCodeModelError"));
        return;
      }
    }
    createAgent.mutate(
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-lg font-semibold">{t("agentsPage.new.heading")}</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {t("agentsPage.new.subtitle")}
        </p>
      </div>
      ...
      <div className="border border-border">
        ...
        <div className="px-4 pt-4 pb-2">
          <input
            ...
            placeholder={t("agentsPage.new.namePlaceholder")}
            value={name}
        ...
        <div className="px-4 pb-2">
          <input
            ...
            placeholder={t("agentsPage.new.titlePlaceholder")}
            value={title}
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">{t("agentsPage.new.companySkills")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("agentsPage.new.companySkillsHint")}
              </p>
            </div>
            {availableSkills.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t("agentsPage.new.noSkills")}
              </p>
            ) : (
              <div className="space-y-3">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("agentsPage.new.willBeCeo")}</p>
          )}
          {formError && (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {testAgentState.pending ? t("agentsPage.new.testing") : t("agentsPage.new.testAgent")}
                </Button>
                <Button
                  size="sm"
                ...
                >
                  {createAgent.isPending ? t("agentsPage.new.creating") : t("agentsPage.new.createAgent")}
                </Button>
              </div>
            </div>
          </div>
