...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyData.export.previewPane.selectFile")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyData.export.previewPane.binaryUnavailable")}
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companyData.export.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.export.toasts.exportFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.export.toasts.loadFailedBody"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyData.export.toasts.downloadedTitle"),
        body: t(resultCheckedFiles.size === 1 ? "companyData.export.toasts.downloadedBodyOne" : "companyData.export.toasts.downloadedBodyOther", { count: resultCheckedFiles.size, name: result.rootPath }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.export.toasts.exportFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.export.toasts.buildFailedBody"),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("companyData.export.selectCompany")} />;
  }
  ...
  if (!exportData) {
    return <EmptyState icon={Package} message={t("companyData.export.loading")} />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {t("companyData.export.exportLabel", { name: selectedCompany?.name ?? t("companyData.export.companyFallback") })}
            </span>
            <span className="text-muted-foreground">
              {t(totalFiles === 1 ? "companyData.export.fileSelectedCountOne" : "companyData.export.fileSelectedCountOther", { selected: selectedCount, total: totalFiles })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t(warnings.length === 1 ? "companyData.export.warningCountOne" : "companyData.export.warningCountOther", { count: warnings.length })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("companyData.export.downloadButtonLoading")
              : t(selectedCount === 1 ? "companyData.export.downloadButtonOne" : "companyData.export.downloadButtonOther", { count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:h-[calc(100vh-12rem)] xl:grid-cols-[19rem_minmax(0,1fr)] xl:gap-0">
        <aside className="flex max-h-[24rem] flex-col overflow-hidden border-b border-border xl:max-h-none xl:border-b-0 xl:border-r">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">{t("companyData.export.packageFiles")}</h2>
          </div>
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("companyData.export.searchPlaceholder")}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("companyData.export.showMoreTasks", { visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
