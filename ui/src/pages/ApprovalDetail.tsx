...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvals.detail.breadcrumbFallback") },
    ]);
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("approvals.detail.cta.reviewLinkedTasks")
              : t("approvals.detail.cta.reviewLinkedTask"),
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approvals.detail.banner.confirmed")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvals.detail.banner.confirmedDescription")}
                </p>
              </div>
            </div>
            <Button
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              <Trans
                i18nKey="approvals.detail.budgetStop"
                components={[<Link to="/costs" className="underline underline-offset-2" />]}
              />
            </p>
          )}
