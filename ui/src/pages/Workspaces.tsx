...
export function Workspaces() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("workspaces.list.breadcrumb") }]);
  }, [setBreadcrumbs, t]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold">{t("workspaces.list.heading")}</h2>
      </div>

      {groups.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("workspaces.list.empty")}</p>
      ) : (
            ...
            <section key={group.project.id} className="space-y-3">
              <div className="flex flex-wrap items-end justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t(group.summaries.length === 1 ? "workspaces.list.workspaceCountOne" : "workspaces.list.workspaceCountOther", { count: group.summaries.length })}
                </span>
              </div>
