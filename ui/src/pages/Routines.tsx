...
const concurrencyPolicyDescriptionKeys: Record<string, string> = {
  coalesce_if_active: "routines.list.composer.concurrencyDescriptions.coalesce_if_active",
  always_enqueue: "routines.list.composer.concurrencyDescriptions.always_enqueue",
  skip_if_active: "routines.list.composer.concurrencyDescriptions.skip_if_active",
};
const catchUpPolicyDescriptionKeys: Record<string, string> = {
  skip_missed: "routines.list.composer.catchUpDescriptions.skip_missed",
  enqueue_missed_with_cap: "routines.list.composer.catchUpDescriptions.enqueue_missed_with_cap",
};
...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routines.common.breadcrumb") }]);
  }, [setBreadcrumbs, t]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.list.toasts.created.title"),
        body: routine.assigneeAgentId
          ? t("routines.list.toasts.created.bodyWithAgent")
          : t("routines.list.toasts.created.bodyDraft"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.list.toasts.updateFailed.title"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.list.toasts.updateFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.common.toasts.runFailed.title"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.common.toasts.runFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const routineGroupLabels = useMemo<RoutineGroupLabels>(
    () => ({
      noProject: t("routines.list.groups.noProject"),
      unknownProject: t("routines.list.groups.unknownProject"),
      unassigned: t("routines.list.groups.unassigned"),
      unknownAgent: t("routines.list.groups.unknownAgent"),
    }),
    [t],
  );
  const routineGroups = useMemo(
    () => buildRoutineGroups(sortedRoutines, routineViewState.groupBy, projectById, agentById, routineGroupLabels),
    [agentById, projectById, routineGroupLabels, routineViewState.groupBy, sortedRoutines],
  );
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines.common.toasts.defaultAgentRequired.title"),
        body: t("routines.common.toasts.defaultAgentRequired.body"),
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routines.common.selectCompany")} />;
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">
            {t("routines.common.breadcrumb")}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("routines.list.subtitle")}
          </p>
        </div>
        <Button onClick={() => setComposerOpen(true)}>
          <Plus className="mr-2 h-4 w-4" />
          {t("routines.list.createRoutine")}
        </Button>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t(visibleRoutines.length === 1 ? "routines.list.routineCountOne" : "routines.list.routineCountOther", { count: visibleRoutines.length })}
            </p>
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.list.toolbar.sort")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.list.toolbar.sort")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routines.list.sortFields.updated")],
                      ["created", t("routines.list.sortFields.created")],
                      ["lastRun", t("routines.list.sortFields.lastRun")],
                      ["title", t("routines.list.sortFields.title")],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routines.list.sortDir.asc") : t("routines.list.sortDir.desc")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.list.toolbar.group")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.list.toolbar.group")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      ["assignee", t("routines.list.groupBy.assignee")],
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routines.list.composer.eyebrow")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routines.list.composer.subtitle")}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routines.common.titlePlaceholder")}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("routines.list.composer.for")}</span>
                  <InlineEntitySelector
                    ref={assigneeSelectorRef}
                    value={draft.assigneeAgentId}
                    options={assigneeOptions}
                    recentOptionIds={recentAssigneeIds}
                    placeholder={t("routines.common.assignee")}
                    noneLabel={t("routines.common.noAssignee")}
                    searchPlaceholder={t("routines.common.searchAssignees")}
                    emptyMessage={t("routines.common.noAssigneesFound")}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.common.assignee")}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  />
                  <span>{t("routines.list.composer.in")}</span>
                  <InlineEntitySelector
                    ref={projectSelectorRef}
                    value={draft.projectId}
                    options={projectOptions}
                    recentOptionIds={recentProjectIds}
                    placeholder={t("routines.common.project")}
                    noneLabel={t("routines.common.noProject")}
                    searchPlaceholder={t("routines.common.searchProjects")}
                    emptyMessage={t("routines.common.noProjectsFound")}
                    onChange={(projectId) => {
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.common.project")}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={t("routines.common.addInstructions")}
                bordered={false}
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines.list.composer.advancedTitle")}</p>
                    <p className="text-sm text-muted-foreground">{t("routines.list.composer.advancedSubtitle")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.list.composer.concurrency")}</p>
                      <Select
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.list.composer.catchUp")}</p>
                      <Select
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.list.composer.footerHint")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routines.list.composer.creating") : t("routines.list.createRoutine")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.list.composer.createFailed")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routines.list.loadError")}
          </CardContent>
        </Card>
      ) : null}
