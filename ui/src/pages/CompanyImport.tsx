...
/** Localized label for an import plan action (create/update/skip/etc.). */
function importActionLabel(t: TFunction, action: string): string {
  return t(`companyData.import.actions.${action}`, { defaultValue: action });
}

/** Localized label for a conflicting entity kind (agent/project/issue/skill). */
function importKindLabel(t: TFunction, kind: string): string {
  return t(`companyData.import.kinds.${kind}`, { defaultValue: kind });
}
...
function renderImportFileExtra(t: TFunction, node: FileTreeNode, checked: boolean, renameMap: Map<string, string>) {
  ...
  const actionBadge = node.action ? (
    <span className={cn(
      ...
      {checked ? importActionLabel(t, node.action) : importActionLabel(t, "skip")}
    </span>
  ) : null;
...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyData.import.previewPane.selectFile")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyData.import.previewPane.binaryUnavailable")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("companyData.import.conflicts.title")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t(conflicts.length === 1 ? "companyData.import.conflicts.itemCountOne" : "companyData.import.conflicts.itemCountOther", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("companyData.import.conflicts.skipped") : t("companyData.import.conflicts.skip")}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("companyData.import.conflicts.confirmed")}
                      </>
                    ) : (
                      t("companyData.import.conflicts.confirmRename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("companyData.import.adapters.title")}</h3>
          <span className="text-xs text-muted-foreground">
            {t(agents.length === 1 ? "companyData.import.adapters.agentCountOne" : "companyData.import.adapters.agentCountOther", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  <span className={cn(
                    ...
                    {t("companyData.import.adapters.agentBadge")}
                  </span>
                  ...
                  >
                    ...
                    {t("companyData.import.adapters.configure")}
                  </button>
                </div>
                {isExpanded && (
...
async function readLocalPackageZip(t: TFunction, file: File): Promise<{
  name: string;
  rootPath: string | null;
  files: Record<string, CompanyPortabilityFileEntry>;
}> {
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("companyData.import.errors.selectZip"));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("companyData.import.errors.noPackageFiles"));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText = t("companyData.import.localZipHelpText");

  useEffect(() => {
    setBreadcrumbs([
      { label: t("org.breadcrumb"), href: "/org" },
      { label: t("companyData.import.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("companyData.import.errors.noSource"));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.import.toasts.previewFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.import.toasts.previewFailedBody"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyData.import.toasts.importCompleteTitle"),
        body: t(result.agents.length === 1 ? "companyData.import.toasts.importCompleteBodyOne" : "companyData.import.toasts.importCompleteBodyOther", { name: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyData.import.toasts.importFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.import.toasts.importFailedBody"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const pkg = await readLocalPackageZip(t, fileList[0]!);
      setLocalPackage(pkg);
      setImportPreview(null);
    } catch (err) {
      pushToast({
        tone: "error",
        title: t("companyData.import.toasts.packageReadFailedTitle"),
        body: err instanceof Error ? err.message : t("companyData.import.toasts.packageReadFailedBody"),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("companyData.import.selectCompany")} />;
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("companyData.import.source.title")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("companyData.import.source.subtitle")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("companyData.import.sourceModes.github") },
              { key: "local", icon: Upload, label: t("companyData.import.sourceModes.local") },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("companyData.import.chooseZip")}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t(Object.keys(localPackage.files).length === 1 ? "companyData.import.packageSummaryOne" : "companyData.import.packageSummaryOther", { name: localPackage.name, count: Object.keys(localPackage.files).length })}
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <Field
            label={t("companyData.import.fields.githubUrl.label")}
            hint={t("companyData.import.fields.githubUrl.hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("companyData.import.fields.githubUrl.placeholder")}
              onChange={(e) => {
        ...
        <Field label={t("companyData.import.fields.target.label")} hint={t("companyData.import.fields.target.hint")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("companyData.import.fields.target.newOption")}</option>
            <option value="existing">
              {t("companyData.import.fields.target.existingOption", { name: selectedCompany?.name ?? "" })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("companyData.import.fields.newName.label")}
            hint={t("companyData.import.fields.newName.hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("companyData.import.fields.newName.placeholder")}
            />
          </Field>
        )}

        <Field
          label={t("companyData.import.fields.collision.label")}
          hint={t("companyData.import.fields.collision.hint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("companyData.import.fields.collision.rename")}</option>
            <option value="skip">{t("companyData.import.fields.collision.skip")}</option>
            <option value="replace">{t("companyData.import.fields.collision.replace")}</option>
          </select>
        </Field>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("companyData.import.previewButtonLoading") : t("companyData.import.previewButton")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("companyData.import.bar.previewLabel")}
              </span>
              <span className="text-muted-foreground">
                {t(totalFiles === 1 ? "companyData.import.fileSelectedCountOne" : "companyData.import.fileSelectedCountOther", { selected: selectedCount, total: totalFiles })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t(conflicts.length === 1 ? "companyData.import.conflictCountOne" : "companyData.import.conflictCountOther", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t(importPreview.errors.length === 1 ? "companyData.import.errorCountOne" : "companyData.import.errorCountOther", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex flex-wrap justify-end gap-2">
            ...
            >
              ...
              {importMutation.isPending
                ? t("companyData.import.importButtonLoading")
                : t(selectedCount === 1 ? "companyData.import.importButtonOne" : "companyData.import.importButtonOther", { count: selectedCount })}
            </Button>
          </div>
          ...
          <div className="grid gap-4 xl:h-[calc(100vh-16rem)] xl:grid-cols-[19rem_minmax(0,1fr)] xl:gap-0">
            <aside className="flex max-h-[24rem] flex-col overflow-hidden border-b border-border xl:max-h-none xl:border-b-0 xl:border-r">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("companyData.import.packageFiles")}</h2>
              </div>
