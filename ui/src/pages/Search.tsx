...
function scopeLabel(t: TFunction, scope: CompanySearchScope): string {
  return t(`search.scopes.${scope}`);
}
...
function subgroupLabel(t: TFunction, key: SubGroupKey): string {
  return t(`search.scopes.${key}`);
}
...
function shapeError(t: TFunction, error: unknown): { message: string; status?: number } {
  if (!error) return { message: t("search.error.unknown") };
...
export function Search() {
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {scopeLabel(t, value as CompanySearchScope)}
  ...
  const apiError = hasError ? shapeError(t, error) : null;
    ...
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        ...
        {recentSearches.length > 0 ? (
          <div>
            <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              {t("search.initial.recentSearches")}
            </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <Trans
              i18nKey="search.initial.identifierTip"
              components={[
          ...
          <li>
            <Trans
              i18nKey="search.initial.quotedTip"
              components={[<span className="font-medium text-foreground" />]}
            />
          </li>
          <li>
            <Trans
              i18nKey="search.initial.commandTip"
              components={[<span className="font-medium text-foreground" />]}
            />
          </li>
        </ul>
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? t("search.error.serverStatus", { status }) : t("search.error.requestFailed")} {t("search.error.hint")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("search.error.openTasksFilter")}
          </Button>
        </div>
      </div>
    );
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.empty.description", { scope: describeScope(t, scope).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
          ...
          <Button onClick={navigateIssuesFallback} size="sm" variant="ghost">
            {t("search.empty.openTasksFilter")}
          </Button>
        </div>
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          ...
          <li>
            <Trans
              i18nKey="search.empty.tipIdentifier"
              components={[<code className="rounded bg-muted px-1 py-0.5" />]}
            />
          </li>
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {totalResults === 1 ? t("search.results.countOne") : t("search.results.countOther", { count: totalResults })} · {t("search.results.sortedByRelevance")}
        </span>
      ...
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={subgroupLabel(t, group.key)}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={subgroupLabel(t, group.key)}
                trailing={
