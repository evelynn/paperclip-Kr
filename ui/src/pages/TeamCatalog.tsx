...
> = {
  markdown_only: {
    labelKey: "teamCatalog.trust.markdownOnly.label",
    tipKey: "teamCatalog.trust.markdownOnly.tip",
    tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30",
    Icon: ShieldCheck,
  },
  assets: {
    labelKey: "teamCatalog.trust.assets.label",
    tipKey: "teamCatalog.trust.assets.tip",
    tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30",
    Icon: ShieldCheck,
  },
  scripts_executables: {
    labelKey: "teamCatalog.trust.scriptsExecutables.label",
    tipKey: "teamCatalog.trust.scriptsExecutables.tip",
    tone: "text-amber-600 dark:text-amber-300 border-amber-500/30",
    Icon: AlertTriangle,
  },
  external_sources: {
    labelKey: "teamCatalog.trust.externalSources.label",
    tipKey: "teamCatalog.trust.externalSources.tip",
    tone: "text-amber-600 dark:text-amber-300 border-amber-500/30",
    Icon: AlertTriangle,
  },
};
...
> = {
  compatible: { labelKey: "teamCatalog.compat.compatible", tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30" },
  unknown: { labelKey: "teamCatalog.compat.unknown", tone: "text-muted-foreground border-border" },
  invalid: { labelKey: "teamCatalog.compat.invalid", tone: "text-rose-600 dark:text-rose-300 border-rose-500/30" },
};
...
function ProvenanceBadge({ team }: { team: CatalogTeam }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{t("teamCatalog.provenanceBadge.tip")}</TooltipContent>
    </Tooltip>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm font-medium">
        ...
        {t(unsafe.length === 1 ? "teamCatalog.riskBanner.externalSourceOne" : "teamCatalog.riskBanner.externalSourceOther", { count: unsafe.length })}
      </div>
    ...
    <div className="max-h-72 overflow-auto rounded-md border border-border">
      <ul className="divide-y divide-border/60">
        ...
        {team.agentSlugs.length === 0 && (
          <li className="px-3 py-2 text-xs text-muted-foreground">{t("teamCatalog.hierarchy.empty")}</li>
        )}
      </ul>
    </div>
  );
...
export function EnvInputsList({ inputs }: { inputs: CatalogTeamEnvInputSummary[] }) {
  ...
  return (
    <div className="space-y-1.5">
      <SectionHeader>{t("teamCatalog.envInputs.section")}</SectionHeader>
      <ul className="space-y-1">
        {inputs.map((input) => (
          <li
            key={input.key}
          ...
          >
            ...
            {input.requirement === "required" && (
              <Badge variant="outline" className="text-[10px]">{t("teamCatalog.envInputs.required")}</Badge>
            )}
          </li>
        ))}
      </ul>
...
export function ExternalSourcesList({ sources }: { sources: CatalogTeamSourceRef[] }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        {open ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
        {t("teamCatalog.externalSources.toggle", { count: external.length })}
      </button>
      {open && (
...
export function TeamDetailPane({
  ...
  const installButton = (
    ...
    >
      ...
      {isInstalled ? t("teamCatalog.detail.installButton.reinstall") : t("teamCatalog.detail.installButton.install")}
    </Button>
  );
    ...
    <div className="flex-1 overflow-auto">
      <div className="space-y-5 p-5">
        ...
        <div className="space-y-2">
          <SectionHeader>{t("teamCatalog.detail.sections.agentHierarchy")}</SectionHeader>
        ...
        {team.projectSlugs.length > 0 && (
          <div className="space-y-2">
            <SectionHeader>{t("teamCatalog.detail.sections.projects")}</SectionHeader>
            <ul className="space-y-1">
        ...
        {/* Required skills */}
        <div className="space-y-2">
          <SectionHeader>{t("teamCatalog.detail.sections.requiredSkills")}</SectionHeader>
        ...
        {/* File inventory */}
        <div className="space-y-2">
          <SectionHeader>{t("teamCatalog.detail.sections.files")}</SectionHeader>
...
const STEP_LABEL_KEYS: Record<WizardStep, string> = {
  target_manager: "teamCatalog.steps.targetManager",
  source_policy: "teamCatalog.steps.sourcePolicy",
  skill_plan: "teamCatalog.steps.skillPlan",
  preview: "teamCatalog.steps.preview",
};
...
export function useInstallTeamCatalogEntry({
  ...
  const previewMutation = useMutation({
    ...
    onError: (error) => {
      setPreviewError(error instanceof Error ? error.message : t("teamCatalog.installer.errors.previewFailed"));
    },
  });
  ...
  const installMutation = useMutation({
    ...
    onError: (error) => {
      setPhase("error");
      setApplyError(error instanceof Error ? error.message : t("teamCatalog.installer.errors.installFailed"));
    },
  });
...
function TeamInstallerDialog({
  ...
  const headerTitle = (
    <span className="flex items-center gap-2">
      ...
      {t("teamCatalog.installer.headerTitle", { name: team.name })}
    </span>
  );
  const headerDescription =
    phase === "form" ? (
      <span className="flex items-center gap-2">
        <span>
          {t("teamCatalog.installer.progress", { current: stepIndex + 1, total: totalSteps, step: t(STEP_LABEL_KEYS[currentStep]) })}
        </span>
        ...
        {phase === "error" && (
          <div className="space-y-3">
            <div role="alert" className="flex items-start gap-2 rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2.5 text-sm text-rose-700 dark:text-rose-300">
              ...
              <div>
                <p className="font-medium">{t("teamCatalog.installer.applyError.title")}</p>
                <p className="mt-0.5 text-xs">{applyError}</p>
                <p className="mt-1 text-xs opacity-80">
                  {t("teamCatalog.installer.applyError.note")}
                </p>
              </div>
            </div>
          </div>
  ...
  const footer =
    phase === "form" ? (
      <div className="flex items-center justify-between gap-3">
        <div>
          {stepIndex > 0 ? (
            <Button variant="ghost" onClick={goBack}>{t("teamCatalog.installer.footer.back")}</Button>
          ) : (
            <Button variant="ghost" onClick={onClose}>{t("teamCatalog.installer.footer.cancel")}</Button>
          )}
        </div>
        <div className="flex items-center gap-3">
          {currentStep === "preview" && hasErrors && (
            <span className="text-xs text-rose-600 dark:text-rose-300">
              {t(blockedCount === 1 ? "teamCatalog.installer.footer.blockedErrorsOne" : "teamCatalog.installer.footer.blockedErrorsOther", { count: blockedCount })}
            </span>
          )}
          {currentStep === "preview" && !hasErrors && missingRequiredSecretCount > 0 && (
            <span className="text-xs text-rose-600 dark:text-rose-300">
              {t("teamCatalog.installer.footer.missingSecrets", { count: missingRequiredSecretCount })}
            </span>
          )}
          {currentStep === "preview" ? (
            needsScriptsConfirm && confirmScripts ? (
              <Button variant="destructive" onClick={submitInstall} disabled={installBlocked || previewMutation.isPending}>
                <AlertTriangle className="h-4 w-4" />
                {t("teamCatalog.installer.footer.confirmExecutables")}
              </Button>
            ) : (
              <Button onClick={submitInstall} disabled={installBlocked || previewMutation.isPending || !previewResult}>
                ...
                {needsScriptsConfirm ? t("teamCatalog.installer.footer.installWithExecutables") : t("teamCatalog.installer.footer.installTeam")}
              </Button>
            )
          ) : (
            <Button onClick={goNext} disabled={!canContinue(currentStep)}>{t("teamCatalog.installer.footer.continue")}</Button>
          )}
        </div>
      </div>
    ) : phase === "error" ? (
      <div className="flex justify-end">
        <Button variant="ghost" onClick={onClose}>{t("teamCatalog.installer.footer.close")}</Button>
      </div>
    ) : null;
...
export function StepTargetManager({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {t("teamCatalog.targetManagerStep.help")}
      </div>

      <div className="space-y-1.5">
        <SectionHeader>{t("teamCatalog.targetManagerStep.rootAgents")}</SectionHeader>
      ...
      {!fullCompany && (
        <div className="space-y-1.5" aria-describedby="target-manager-help">
          <SectionHeader>{t("teamCatalog.targetManagerStep.targetManager")}</SectionHeader>
          <Command className="rounded-md border border-border">
            <CommandInput placeholder={t("teamCatalog.targetManagerStep.searchPlaceholder")} />
            <CommandList>
              <CommandEmpty>{t("teamCatalog.targetManagerStep.noAgents")}</CommandEmpty>
              <CommandGroup>
                {agents.map((agent) => (
                  <CommandItem
                    key={agent.id}
      ...
      {canBypassManager && (
        <label className="flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm">
          ...
          />
          {t("teamCatalog.targetManagerStep.fullCompany")}
        </label>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div role="alert" className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-700 dark:text-amber-300">
        {t(external.length === 1 ? "teamCatalog.sourcePolicyStep.warningOne" : "teamCatalog.sourcePolicyStep.warningOther", { count: external.length })}
      </div>
            ...
            <li key={`${source.type}:${source.ref}`} className="flex items-center gap-2 px-3 py-2.5 text-sm">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {code === "ok" && t("teamCatalog.sourcePolicyStep.pinned")}
                  {code === "unpinned" && t("teamCatalog.sourcePolicyStep.unpinned")}
                  {code === "unsupported_in_ui" && t("teamCatalog.sourcePolicyStep.unsupported")}
                </p>
              </div>
              <Badge
                variant="outline"
      ...
      <div className="space-y-2.5 rounded-md border border-border p-3">
        <PolicyToggle
          label={t("teamCatalog.sourcePolicyStep.allowExternal.label")}
          description={t("teamCatalog.sourcePolicyStep.allowExternal.description")}
          checked={allowExternalSources}
          onChange={(v) => onChange("external", v)}
        />
        <PolicyToggle
          label={t("teamCatalog.sourcePolicyStep.allowUnpinned.label")}
          description={t("teamCatalog.sourcePolicyStep.allowUnpinned.description")}
          checked={allowUnpinnedOptionalSources}
          onChange={(v) => onChange("unpinned", v)}
        />
        <PolicyToggle
          label={t("teamCatalog.sourcePolicyStep.allowLocalPath.label")}
          description={t("teamCatalog.sourcePolicyStep.allowLocalPath.description")}
      ...
      {hasUnsupported && !allowLocalPathSources && (
        <p className="text-xs text-rose-600 dark:text-rose-300">
          {t("teamCatalog.sourcePolicyStep.localPathHint")}
        </p>
      )}
    </div>
  );
...
const SKILL_ACTION_META: Record<
  ...
  { labelKey: string; tone: string }
> = {
  already_in_package: { labelKey: "teamCatalog.skillAction.alreadyInPackage", tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30" },
  catalog_install_required: { labelKey: "teamCatalog.skillAction.catalogInstallRequired", tone: "text-blue-600 dark:text-blue-300 border-blue-500/30" },
  external_import_required: { labelKey: "teamCatalog.skillAction.externalImportRequired", tone: "text-amber-600 dark:text-amber-300 border-amber-500/30" },
  blocked: { labelKey: "teamCatalog.skillAction.blocked", tone: "text-rose-600 dark:text-rose-300 border-rose-500/30" },
};
  ...
  return (
    <div className="space-y-4">
      <div role="alert" className="rounded-md border border-blue-500/30 bg-blue-500/10 px-3 py-2.5 text-sm text-blue-700 dark:text-blue-300">
        {t("teamCatalog.skillPlanStep.intro")}
      </div>
      <ul className="divide-y divide-border rounded-md border border-border">
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
          <SummaryCount label={t("teamCatalog.preview.summaryCounts.agents")} value={plan.agentPlans.length} />
          <SummaryCount label={t("teamCatalog.preview.summaryCounts.projects")} value={plan.projectPlans.length} />
          <SummaryCount label={t("teamCatalog.preview.summaryCounts.starterTasks")} value={plan.issuePlans.length} />
          <SummaryCount label={t("teamCatalog.preview.summaryCounts.requiredSkills")} value={result.skillPreparations.length} />
        </div>
      </div>
      ...
      {result.errors.length > 0 && (
        <div role="alert" className="rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2.5 text-sm text-rose-700 dark:text-rose-300">
          <p className="font-medium">{t("teamCatalog.preview.installBlocked")}</p>
      ...
      {plan.agentPlans.length > 0 && (
        <PreviewSection title={t("teamCatalog.preview.agentsSection", { count: plan.agentPlans.length })}>
      ...
      {plan.projectPlans.length > 0 && (
        <PreviewSection title={t("teamCatalog.preview.projectsSection", { count: plan.projectPlans.length })}>
      ...
      {plan.issuePlans.length > 0 && (
        <PreviewSection title={t("teamCatalog.preview.starterTasksSection", { count: plan.issuePlans.length })}>
      ...
      {manifestAgents.length > 0 && (
        <PreviewSection title={t("teamCatalog.preview.adapterSection", { count: manifestAgents.length })}>
      ...
      {/* Env inputs */}
      {envInputs.length > 0 && (
        <PreviewSection title={t("teamCatalog.preview.envInputsSection", { count: envInputs.length })}>
          {envInputs.map((input) => {
            ...
            return (
              <li key={formKey} className="grid gap-2 px-3 py-2 text-sm sm:grid-cols-[minmax(0,1fr)_minmax(14rem,18rem)] sm:items-center">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  {input.requirement === "required" && (
                    <Badge variant="outline" className="text-[10px]">{t("teamCatalog.preview.envInput.required")}</Badge>
                  )}
                  <Badge
                    variant="outline"
                ...
                <div className="flex items-center gap-1.5">
                  <Input
                    ...
                    placeholder={input.requirement === "required" ? t("teamCatalog.preview.envInput.requiredPlaceholder") : t("teamCatalog.preview.envInput.optionalPlaceholder")}
                    aria-label={t("teamCatalog.preview.envInput.valueAria", { key: input.key })}
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        aria-label={visible ? t("teamCatalog.preview.envInput.hideAria", { key: input.key }) : t("teamCatalog.preview.envInput.showAria", { key: input.key })}
                      >
                        {visible ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>{visible ? t("teamCatalog.preview.envInput.hideValue") : t("teamCatalog.preview.envInput.showValue")}</TooltipContent>
                  </Tooltip>
                </div>
              </li>
            );
      ...
      <div className="rounded-md border border-border px-3 py-2.5 text-xs text-muted-foreground">
        <Trans
          i18nKey="teamCatalog.preview.provenance"
          values={{ provenance: team.packageName ?? team.key, hash: team.contentHash.slice(0, 16) }}
          components={[<code className="font-mono" />, <code className="font-mono" />]}
        />
      </div>
    </div>
  );
...
export function ApplyProgress({ team }: { team: CatalogTeam }) {
  ...
  return (
    <div className="flex items-center gap-3 py-10 text-sm">
      ...
      <div>
        <p className="font-medium">{t("teamCatalog.apply.installing", { name: team.name })}</p>
        <p className="text-xs text-muted-foreground">
          {t("teamCatalog.apply.installingNote")}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      <p className="text-sm text-muted-foreground">
        {t("teamCatalog.success.summary", { name: team.name })}
      </p>
      {result && (
      ...
      <div className="flex items-center gap-2">
        ...
        {outOfDate && (
          <Tooltip>
            <TooltipTrigger asChild>
              <span
                aria-label={t("teamCatalog.row.updateAvailable")}
            ...
            <TooltipContent>{t("teamCatalog.row.updateAvailableTip")}</TooltipContent>
          </Tooltip>
        )}
        {risk !== "safe" && (
          <Tooltip>
            ...
            <TooltipContent>{t("teamCatalog.row.hasExternalSources")}</TooltipContent>
          </Tooltip>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          {t(team.counts.agents === 1 ? "teamCatalog.card.agentsOne" : "teamCatalog.card.agentsOther", { count: team.counts.agents })} ·{" "}
          {t(team.counts.projects === 1 ? "teamCatalog.card.projectsOne" : "teamCatalog.card.projectsOther", { count: team.counts.projects })} ·{" "}
          {t(team.counts.routines === 1 ? "teamCatalog.card.routinesOne" : "teamCatalog.card.routinesOther", { count: team.counts.routines })}
        </p>
      </div>
...
export function TeamCatalog() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("teamCatalog.page.breadcrumbOrgChart"), href: "/org" },
      { label: t("teamCatalog.page.breadcrumbTeams"), href: TEAM_CATALOG_ROUTE_ROOT },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return (
      <div className="p-8">
        <EmptyState icon={Users2} message={t("teamCatalog.page.selectCompany")} />
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border px-5 py-3">
        <h1 className="text-lg font-semibold">{t("teamCatalog.list.title")}</h1>
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
          <Input
            value={q}
            onChange={(e) => setFilterParam("search", e.target.value)}
            placeholder={t("teamCatalog.list.searchPlaceholder")}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-8">
              ...
              {kindFilter === "all" ? t("teamCatalog.list.allKinds") : kindFilter === "bundled" ? t("teamCatalog.list.bundled") : t("teamCatalog.list.optional")}
              <ChevronDown className="h-3.5 w-3.5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start">
            <DropdownMenuLabel>{t("teamCatalog.list.kind")}</DropdownMenuLabel>
            <DropdownMenuRadioGroup value={kindFilter} onValueChange={(v) => setFilterParam("kind", v)}>
              <DropdownMenuRadioItem value="all">{t("teamCatalog.list.allKinds")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="bundled">{t("teamCatalog.list.bundled")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="optional">{t("teamCatalog.list.optional")}</DropdownMenuRadioItem>
            </DropdownMenuRadioGroup>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        {categories.length > 0 && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-8">
                {categoryFilter ? t("teamCatalog.list.categoryLabel", { category: titleCase(categoryFilter) }) : t("teamCatalog.list.allCategories")}
                <ChevronDown className="h-3.5 w-3.5" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start">
              <DropdownMenuLabel>{t("teamCatalog.list.category")}</DropdownMenuLabel>
              <DropdownMenuRadioGroup value={categoryFilter} onValueChange={(v) => setFilterParam("category", v)}>
                <DropdownMenuRadioItem value="">{t("teamCatalog.list.allCategories")}</DropdownMenuRadioItem>
                {categories.map((cat) => (
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-8">
              {riskFilter === "any" ? t("teamCatalog.list.anyRisk") : riskFilter === "safe" ? t("teamCatalog.list.safeOnly") : riskFilter === "has_warnings" ? t("teamCatalog.list.hasWarnings") : t("teamCatalog.list.blocked")}
              <ChevronDown className="h-3.5 w-3.5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start">
            <DropdownMenuLabel>{t("teamCatalog.list.risk")}</DropdownMenuLabel>
            <DropdownMenuRadioGroup value={riskFilter} onValueChange={(v) => setFilterParam("risk", v)}>
              <DropdownMenuRadioItem value="any">{t("teamCatalog.list.anyRisk")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="safe">{t("teamCatalog.list.safeOnly")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="has_warnings">{t("teamCatalog.list.hasWarnings")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="blocked">{t("teamCatalog.list.blocked")}</DropdownMenuRadioItem>
            </DropdownMenuRadioGroup>
            {anyFilterActive && (
              <>
                <DropdownMenuSeparator />
                ...
                >
                  <RotateCcw className="h-3 w-3" /> {t("teamCatalog.list.resetFilters")}
                </button>
              </>
            )}
          </DropdownMenuContent>
        ...
        {anyFilterActive && (
          <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => setSearchParams(new URLSearchParams())}>
            {t("teamCatalog.list.resetFilters")}
          </Button>
        )}
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          {catalogQuery.isLoading ? (
            ...
            <div className="p-4">
              <div role="alert" className="rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2.5 text-sm text-rose-700 dark:text-rose-300">
                {t("teamCatalog.list.loadFailed")}
              </div>
              <Button variant="outline" size="sm" className="mt-3" onClick={() => catalogQuery.refetch()}>
                <RotateCcw className="h-3.5 w-3.5" /> {t("teamCatalog.list.retry")}
              </Button>
            </div>
          ) : teams.length === 0 ? (
            <EmptyState icon={Users2} message={t("teamCatalog.list.noneConfigured")} />
          ) : filtered.length === 0 ? (
            <EmptyState
              icon={Search}
              message={t("teamCatalog.list.noMatch")}
              action={t("teamCatalog.list.resetFilters")}
            ...
            <div>
              {grouped.bundled.length > 0 && (
                <>
                  <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("teamCatalog.list.bundledGroup", { count: grouped.bundled.length })}
                  </div>
              ...
              {grouped.optional.length > 0 && (
                <>
                  <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("teamCatalog.list.optionalGroup", { count: grouped.optional.length })}
                  </div>
              ...
              {grouped.installed.length > 0 && (
                <>
                  <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("teamCatalog.list.installedGroup", { count: grouped.installed.length })}
                  </div>
        ...
        {(isDesktop || selectedTeam) && (
          ...
          >
            {!isDesktop && selectedTeam && (
              ...
              >
                <ChevronLeft className="h-4 w-4" /> {t("teamCatalog.page.backToCatalog")}
              </button>
            )}
            {selectedTeam ? (
              <TeamDetailPane
      ...
      {selectedTeam && installOpen && (
        <TeamInstallerDialog
          ...
          onInstalled={() => {
            pushToast({ tone: "success", title: t("teamCatalog.toast.installedTitle"), body: t("teamCatalog.toast.installedBody", { name: selectedTeam.name }) });
