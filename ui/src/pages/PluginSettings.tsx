...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("settings.common.company"), href: "/dashboard" },
      ...
      { label: t("settings.sidebar.plugins"), href: "/company/settings/instance/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("plugins.settings.detailsBreadcrumb") },
    ]);
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("plugins.settings.noDescription");
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("plugins.settings.about.heading")}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("plugins.settings.about.description")}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("plugins.settings.about.author")}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("plugins.settings.about.categories")}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("plugins.settings.about.none")}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("plugins.settings.section.settings")}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("plugins.settings.environments.configureFromInstance")}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("plugins.settings.environments.description", { driverLabel: driverLabel || t("plugins.settings.environments.thisPlugin") })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/instance/environments">
                      <Button variant="outline" size="sm">{t("plugins.settings.environments.open")}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("plugins.settings.status.runtimeDashboard")}
                  </CardTitle>
                  <CardDescription>
                    {t("plugins.settings.status.runtimeDescription")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins.settings.status.workerProcess")}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.status.workerStatus")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.status.pid")}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.status.uptime")}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins.settings.status.pendingRpcs")}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("plugins.settings.status.crashes")}
                                  </span>
                                  <span className="text-xs">
                                    {t("plugins.settings.status.crashesValue", { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("plugins.settings.status.lastCrash")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins.settings.status.noWorker")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins.settings.status.recentJobRuns")}
                        </h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins.settings.status.noJobRuns")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins.settings.status.recentWebhookDeliveries")}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins.settings.status.noWebhookDeliveries")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("plugins.settings.status.lastChecked", { time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("plugins.settings.status.runtimeUnavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("plugins.settings.status.recentLogs")}
                    </CardTitle>
                    <CardDescription>{t(recentLogs.length === 1 ? "plugins.settings.status.logEntriesOne" : "plugins.settings.status.logEntriesOther", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("plugins.settings.status.healthStatus")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("plugins.settings.status.checkingHealth")}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("plugins.settings.status.overall")}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("plugins.settings.status.lifecycle")}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("plugins.settings.status.healthAfterReady")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("plugins.settings.status.details")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.status.pluginId")}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.status.pluginKey")}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.status.npmPackage")}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins.settings.status.version")}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("plugins.settings.status.permissions")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("plugins.settings.status.noPermissions")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("plugins.settings.localFolders.selectCompany")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("plugins.settings.localFolders.heading")}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("plugins.settings.localFolders.loadError")}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t("plugins.settings.localFolders.loading")}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("plugins.settings.localFolders.saved")
          : t("plugins.settings.localFolders.savedNeedsAttention"),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("plugins.settings.localFolders.saveError") });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("plugins.settings.localFolders.pathRequired") });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("plugins.settings.localFolders.pathMustBeAbsolute") });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t("plugins.settings.localFolders.healthy") : t("plugins.settings.localFolders.needsAttention")}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t("plugins.settings.localFolders.readWrite") : t("plugins.settings.localFolders.readOnly")}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t("plugins.settings.localFolders.configured")} value={status?.configured ? t("plugins.settings.localFolders.yes") : t("plugins.settings.localFolders.no")} ok={!!status?.configured} />
        <FolderStatusMetric label={t("plugins.settings.localFolders.readable")} value={status?.readable ? t("plugins.settings.localFolders.yes") : t("plugins.settings.localFolders.no")} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t("plugins.settings.localFolders.writable")}
          value={access === "read" ? t("plugins.settings.localFolders.notRequested") : status?.writable ? t("plugins.settings.localFolders.yes") : t("plugins.settings.localFolders.no")}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("plugins.settings.localFolders.configuredPath")}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("plugins.settings.localFolders.pathLabel")}
        </label>
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={t("plugins.settings.localFolders.pathPlaceholder")}
          />
          ...
          >
            ...
            {t("plugins.settings.localFolders.save")}
          </Button>
        </div>
      </div>
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t("plugins.settings.localFolders.validationProblems")}</div>
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={t("plugins.settings.localFolders.requiredDirectories")}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={t("plugins.settings.localFolders.missingDirectories")}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={t("plugins.settings.localFolders.requiredFiles")}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={t("plugins.settings.localFolders.missingFiles")}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-[10px]">
            {t("plugins.settings.localFolders.notInspected")}
          </Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-[10px]">
            {t("plugins.settings.localFolders.missingCount", { count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-[10px]">{t("plugins.settings.localFolders.present")}</Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("plugins.settings.localFolders.noneDeclared")}</p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("plugins.settings.localFolders.rootNotInspected")}</p>
      ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("plugins.settings.configForm.saved") });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("plugins.settings.configForm.saveError") });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("plugins.settings.configForm.testPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("plugins.settings.configForm.testFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("plugins.settings.configForm.testFailed") });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        <Loader2 className="h-4 w-4 animate-spin" />
        {t("plugins.settings.configForm.loading")}
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("plugins.settings.configForm.saving")}
            </>
          ) : (
            t("plugins.settings.configForm.save")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("plugins.settings.configForm.testing")}
              </>
            ) : (
              t("plugins.settings.configForm.test")
            )}
          </Button>
        )}
      </div>
