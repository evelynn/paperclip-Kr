...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.breadcrumbs.heartbeats") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.toggleError"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t("settings.instance.disableOneFailure", { detail })
            : t("settings.instance.disableManyFailure", { failures: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.disableAllError"));
    },
  });
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.instance.title")}</h1>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("settings.instance.active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("settings.instance.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("settings.instance.companyOne") : t("settings.instance.companyOther")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const message = enabledCount === 1
                ? t("settings.instance.confirmDisableAllOne", { count: enabledCount })
                : t("settings.instance.confirmDisableAllOther", { count: enabledCount });
          ...
          >
            {disableAllMutation.isPending ? t("settings.instance.disabling") : t("settings.instance.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("settings.instance.empty")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("settings.instance.on") : t("settings.instance.off")}
                        </Badge>
                        <Link
                        ...
                        <span className="text-muted-foreground tabular-nums shrink-0">
                          {t("settings.instance.intervalSeconds", { count: agent.intervalSec })}
                        </span>
                        <span
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("settings.instance.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving ? t("settings.instance.saving") : agent.heartbeatEnabled ? t("settings.instance.disableTimerHeartbeat") : t("settings.instance.enableTimerHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
