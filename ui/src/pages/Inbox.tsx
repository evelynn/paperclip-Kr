...
function runFailureMessage(run: HeartbeatRun, t?: TFunction): string {
  const fallback = t ? t("inbox.failedRun.runError") : "Run exited with an error.";
...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    if (t) {
      return joinRequest.agentName
        ? t("inbox.joinRequest.agentLabelNamed", { name: joinRequest.agentName })
        : t("inbox.joinRequest.agentLabel");
    }
  ...
  return t ? t("inbox.joinRequest.humanLabel") : "Human join request";
}
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("inbox.row.markRead")}
              >
                <span className={cn(
              ...
              <button
                ...
                aria-label={t("inbox.row.dismissFromInbox")}
              >
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{linkedAgentName ? t("inbox.failedRun.titleWithAgent", { name: linkedAgentName }) : t("inbox.failedRun.title")}</>
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("inbox.failedRun.retrying") : t("inbox.failedRun.retry")}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
              ...
              aria-label={t("inbox.row.dismiss")}
            >
              <X className="h-4 w-4" />
            </button>
          )}
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("inbox.joinRequest.requestedFromIp", { time: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("inbox.joinRequest.adapter", { type: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.approve"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.reject"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.approveJoin"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.rejectJoin"));
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.archive"));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.undoArchive"));
    },
    onSuccess: (_data, id) => {
          ...
          {tab === "blocked" ? (
            <>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="space-y-0.5 p-2">
                    {BLOCKED_GROUP_OPTIONS.map(([value]) => (
                      ...
                      >
                        <span>{t(`blockedReasons.groupOptions.${value}`)}</span>
              ...
              <IssueColumnPicker
                ...
                title={t("inbox.toolbar.columnsTitle")}
                iconOnly
              />
              <Popover>
                <PopoverTrigger asChild>
                ...
                <PopoverContent align="end" className="w-48 p-0">
                  <div className="space-y-0.5 p-2">
                    {BLOCKED_SORT_OPTIONS.map(([value]) => (
                      ...
                      >
                        <span>{t(`blockedReasons.sortOptions.${value}`)}</span>
            ...
            <>
              <Button
                ...
                title={nestingEnabled ? t("inbox.toolbar.disableNesting") : t("inbox.toolbar.enableNesting")}
              >
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-40 p-2">
                  <div className="space-y-0.5">
                    {([
                      ["none", t("inbox.groupBy.none")],
                      ["type", t("inbox.groupBy.type")],
                      ["assignee", t("inbox.groupBy.assignee")],
                      ["project", t("inbox.groupBy.project")],
                      ...(isolatedWorkspacesEnabled ? ([["workspace", t("inbox.groupBy.workspace")]] as const) : []),
              ...
              {canMarkAllRead && (
                <>
                  ...
                  >
                    {markAllReadMutation.isPending ? t("inbox.markAllRead.pending") : t("inbox.markAllRead.button")}
                  </Button>
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        <DialogTitle>{t("inbox.markAllRead.confirmTitle")}</DialogTitle>
                        <DialogDescription>
                          {t(unreadIssueIds.length === 1 ? "inbox.markAllRead.confirmBodyOne" : "inbox.markAllRead.confirmBodyOther", { count: unreadIssueIds.length })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
                        <Button variant="outline" onClick={() => setShowMarkAllReadConfirm(false)}>
                          {t("inbox.markAllRead.cancel")}
                        </Button>
                        <Button
                          onClick={() => {
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("inbox.categoryFilter.placeholder")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("inbox.categoryFilter.everything")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("inbox.categoryFilter.myRecentTasks")}</SelectItem>
              <SelectItem value="join_requests">{t("inbox.categoryFilter.joinRequests")}</SelectItem>
              <SelectItem value="approvals">{t("inbox.categoryFilter.approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("inbox.categoryFilter.failedRuns")}</SelectItem>
              <SelectItem value="alerts">{t("inbox.categoryFilter.alerts")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("inbox.approvalFilter.placeholder")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("inbox.approvalFilter.all")}</SelectItem>
                <SelectItem value="actionable">{t("inbox.approvalFilter.actionable")}</SelectItem>
                <SelectItem value="resolved">{t("inbox.approvalFilter.resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {t(childCount === 1 ? "inbox.group.subTaskCountOne" : "inbox.group.subTaskCountOther", { count: childCount })}
                        </span>
                      ) : undefined}
                    ...
                    elements.push(
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived" ? t("inbox.searchDivider.archived") : t("inbox.searchDivider.other")}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("inbox.group.newTask", { group: group.label })}
                              aria-label={t("inbox.group.newTask", { group: group.label })}
                              onClick={(event) => {
                                event.stopPropagation();
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      <Trans
                        i18nKey={dashboard!.agents.error === 1 ? "inbox.alerts.agentErrorsOne" : "inbox.alerts.agentErrorsOther"}
                  ...
                  <button
                    ...
                    aria-label={t("inbox.alerts.dismiss")}
                  >
                    <X className="h-3.5 w-3.5" />
                  </button>
                </div>
