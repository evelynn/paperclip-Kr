...
function projectWorkspaceBaseTabItems(t: TFunction): OrderedProjectWorkspaceTabItem[] {
  return [
    { value: "configuration", label: t("workspaces.project.tabs.configuration"), order: 30 },
  ];
}
...
const SOURCE_TYPE_KEYS: Record<ProjectWorkspaceSourceType, string> = {
  local_path: "localPath",
  non_git_path: "nonGitPath",
  git_repo: "gitRepo",
  remote_managed: "remoteManaged",
};

const VISIBILITY_KEYS: Record<ProjectWorkspaceVisibility, string> = {
  default: "default",
  advanced: "advanced",
};

function sourceTypeOptions(t: TFunction): Array<{ value: ProjectWorkspaceSourceType; label: string; description: string }> {
  return (Object.keys(SOURCE_TYPE_KEYS) as ProjectWorkspaceSourceType[]).map((value) => ({
    value,
    label: t(`workspaces.project.sourceTypeOptions.${SOURCE_TYPE_KEYS[value]}.label`),
    description: t(`workspaces.project.sourceTypeOptions.${SOURCE_TYPE_KEYS[value]}.description`),
  }));
}

function visibilityOptions(t: TFunction): Array<{ value: ProjectWorkspaceVisibility; label: string }> {
  return (Object.keys(VISIBILITY_KEYS) as ProjectWorkspaceVisibility[]).map((value) => ({
    value,
    label: t(`workspaces.project.visibilityOptions.${VISIBILITY_KEYS[value]}`),
  }));
}
...
function parseRuntimeConfigJson(t: TFunction, value: string) {
  const trimmed = value.trim();
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("workspaces.project.errors.commandsJsonObject"),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("workspaces.project.errors.invalidJson"),
    };
  }
}
...
function buildWorkspacePatch(t: TFunction, initialState: WorkspaceFormState, nextState: WorkspaceFormState) {
  ...
  if (initialState.runtimeConfig !== nextState.runtimeConfig) {
    const parsed = parseRuntimeConfigJson(t, nextState.runtimeConfig);
...
function validateWorkspaceForm(t: TFunction, form: WorkspaceFormState) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return t("workspaces.project.errors.remoteRefOrRepoRequired");
    }
  } else if (!cwd && !repoUrl) {
    return t("workspaces.project.errors.localPathOrRepoRequired");
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return t("workspaces.project.errors.localPathAbsolute");
  }
  ...
  if (repoUrl) {
    try {
      ...
      return t("workspaces.project.errors.repoUrlValid");
    }
  }

  const runtimeConfig = parseRuntimeConfigJson(t, form.runtimeConfig);
  if (!runtimeConfig.ok) {
...
export function ProjectWorkspaceDetail() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("workspaces.list.breadcrumb"), href: `/projects/${canonicalProjectRef}/workspaces` },
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("workspaces.project.errors.saveFailed"));
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("workspaces.project.errors.updateFailed"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("workspaces.project.runtimeActions.jobCompleted")
          : request.action === "stop"
            ? t("workspaces.project.runtimeActions.serviceStopped")
            : request.action === "restart"
              ? t("workspaces.project.runtimeActions.serviceRestarted")
              : t("workspaces.project.runtimeActions.serviceStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("workspaces.project.errors.controlFailed"));
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("workspaces.project.loading")}</p>;
  if (projectQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {projectQuery.error instanceof Error ? projectQuery.error.message : t("workspaces.project.loadFailed")}
      </p>
    );
  }
  ...
  const sourceTypeOptionItems = sourceTypeOptions(t);
  const visibilityOptionItems = visibilityOptions(t);

  const saveChanges = () => {
    const validationError = validateWorkspaceForm(t, form);
    if (validationError) {
      setErrorMessage(validationError);
      return;
    }
    const patch = buildWorkspacePatch(t, initialState, form);
    if (Object.keys(patch).length === 0) return;
    updateWorkspace.mutate(patch);
  };

  const sourceTypeDescription = sourceTypeOptionItems.find((option) => option.value === form.sourceType)?.description ?? null;
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to={`/projects/${canonicalProjectRef}/workspaces`}>
            ...
            {t("workspaces.project.backToWorkspaces")}
          </Link>
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        {!workspace.isPrimary ? (
          ...
          >
            ...
            {t("workspaces.project.makePrimary")}
          </Button>
        ) : (
          <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
            ...
            {t("workspaces.project.isPrimary")}
          </div>
        )}
      </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label={t("workspaces.project.fields.visibility")}>
                ...
                >
                  {visibilityOptionItems.map((option) => (
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("workspaces.project.fields.sourceType")} hint={sourceTypeDescription ?? undefined}>
                ...
                >
                  {sourceTypeOptionItems.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
              </Field>
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("workspaces.project.fields.localPath")}>
                  <input
                    ...
                    placeholder={t("workspaces.project.fields.localPathPlaceholder")}
                  />
                </Field>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.project.fields.repoUrl")}>
                  <input
                    ...
                    placeholder={t("workspaces.project.fields.repoUrlPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.project.fields.repoRef")}>
                  <input
                    ...
                    placeholder={t("workspaces.project.fields.repoRefPlaceholder")}
                  />
                </Field>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.project.fields.defaultRef")}>
                  <input
                    ...
                    placeholder={t("workspaces.project.fields.defaultRefPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.project.fields.sharedWorkspaceKey")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    ...
                    placeholder={t("workspaces.project.fields.sharedWorkspaceKeyPlaceholder")}
                  />
                </Field>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.project.fields.remoteProvider")}>
                  <input
                    ...
                    placeholder={t("workspaces.project.fields.remoteProviderPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.project.fields.remoteWorkspaceRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    ...
                    placeholder={t("workspaces.project.fields.remoteWorkspaceRefPlaceholder")}
                  />
                </Field>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspaces.project.fields.setupCommand")} hint={t("workspaces.project.fields.setupCommandHint")}>
                  <textarea
                    ...
                    placeholder={t("workspaces.project.fields.setupCommandPlaceholder")}
                  />
                </Field>
                <Field label={t("workspaces.project.fields.cleanupCommand")} hint={t("workspaces.project.fields.cleanupCommandHint")}>
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.cleanupCommand}
                    onChange={(event) => setForm((current) => current ? { ...current, cleanupCommand: event.target.value } : current)}
                    placeholder={t("workspaces.project.fields.cleanupCommandPlaceholder")}
                  />
                </Field>
              </div>

              <details className="rounded-xl border border-dashed border-border/70 bg-background px-3 py-3">
                <summary className="cursor-pointer text-sm font-medium">{t("workspaces.project.advancedRuntime.summary")}</summary>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("workspaces.project.advancedRuntime.description")}
                </p>
                <div className="mt-3">
                  <Field label={t("workspaces.project.fields.commandsJson")} hint={t("workspaces.project.fields.commandsJsonHint")}>
                    <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              >
                {t("workspaces.project.reset")}
              </Button>
              {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
              {!errorMessage && runtimeActionMessage ? <p className="text-sm text-muted-foreground">{runtimeActionMessage}</p> : null}
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("workspaces.project.noUnsavedChanges")}</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces.project.facts.eyebrow")}</div>
            ...
            <DetailRow label={t("workspaces.project.facts.localPath")}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("workspaces.project.facts.none")}</span>
            </DetailRow>
            <DetailRow label={t("workspaces.project.facts.repo")}>
              ...
              ) : t("workspaces.project.facts.none")}
            </DetailRow>
            <DetailRow label={t("workspaces.project.facts.defaultRef")}>{workspace.defaultRef ?? t("workspaces.project.facts.none")}</DetailRow>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces.project.commands.eyebrow")}</div>
            ...
            <WorkspaceRuntimeControls
              ...
              serviceEmptyMessage={
                workspace.runtimeConfig?.workspaceRuntime
                  ? t("workspaces.project.commands.serviceEmptyWithConfig")
                  : t("workspaces.project.commands.serviceEmptyNoConfig")
              }
              jobEmptyMessage={t("workspaces.project.commands.jobEmpty")}
              disabledHint={t("workspaces.project.commands.disabledHint")}
      ...
      {isProjectWorkspacePluginTab(activeTab) ? (
        activePluginTab ? (
          ...
          <div className="rounded-lg border border-dashed border-border bg-background px-4 py-8 text-sm text-muted-foreground">
            {pluginDetailSlotsError ? pluginDetailSlotsError : t("workspaces.project.pluginTab.loading")}
          </div>
        ) : (
          <MissingPluginTabPlaceholder
            defaultTabHref={`${projectWorkspaceUrl(project, routeWorkspaceId)}?tab=configuration`}
            defaultTabLabel={t("workspaces.project.pluginTab.backToConfiguration")}
          />
        )
      ) : null}
    </div>
